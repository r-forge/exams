...
But in addition to these materials that we created, many R/exams users stepped up and created really cool and useful materials. Especially from users in Spain came a lot of interest along with great contributions:
...
Not surprisingly, we had been using R/exams extensively in our teaching for a [blended learning approach in a Mathematics 101 course]({{ site.url }}/general/uibk_math/). The asynchronous parts of the course (screencasts, forum, self tests, and formative online tests) were already all delivered through the university's OpenOLAT learning management system. Thus, we "just" had to switch the synchronous parts of the course (lecture, tutorials, and summative written exams) to distance learning formats:
...
The most challenging part were the [summative online exams]({{ site.url }}/tutorials/openolat_exam/) but overall everything worked out reasonably ok. One nice aspect was that other colleagues profited immediately from using a similar approach for their online exams as well. Like my colleague [Lisa Lechner](http://www.lisalechner.com/) from the Department of Political Science
...
## Replacing (large) written exams

Finding a suitable replacement for the summative written exams was not only a challenge for us. One concern that came up several times
was that learning management systems like Moodle might not be reliable enough for online exams, especially if students' internet connections are flaky.
Hence, one idea that was discussed, especially with [Felix Schönbrodt](https://www.nicebread.de/) and [Arthur Charpentier](https://freakonometrics.github.io/),
was to e-mail [fillable PDF forms](https://stackoverflow.com/questions/62392297/create-fillable-pdf-form-with-exams2nops) to students
[instead of relying entirely on Moodle](https://twitter.com/freakonometrics/status/1293985139089911820). However, I could convince them that
using PDF forms would create more problems than they solve (see the linked threads above for more details):
...
> [Ulrike Grömping on R-Forge](https://R-Forge.R-project.org/forum/forum.php?thread_id=34019&forum_id=4377&group_id=1337): I have also run online exams via Moodle in real time, supervised in a video conference [...]. And I have not used auto correction, but have asked questions that also require free text answers from students (text fields with arbitrary correct answer of (e.g.) 72 characters in a cloze exercise). Of course, the solutions support my manual correction process.
...
While overall there is a lot of heterogeneity in the learning management systems that the R/exams community uses, Moodle is clearly the favorite. While Moodle quizzes also have their quirks, they are surprisingly flexible and can be used at various levels.
...
Especially, the cloze question format that allows to combine multiple- and single-choice, numeric, and string items is popular.

> [Rebecca Killick on R-Forge](https://R-Forge.R-project.org/forum/forum.php?thread_id=34019&forum_id=4377&group_id=1337): We moved to using this exclusively for our undergraduate quizzes in the summer of 2019 and this served us well when COVID-19 hit. We are now expanding our online assessment using R/exams. We mainly use cloze questions with randomization.
...
For facilitating their collaboration, the group around Fernando Mayer has even created a [ShinyExams interface](http://shiny.leg.ufpr.br/fernandomayer/rexams/).
