...
include_asset <- function(file, asset = TRUE, screenshot = TRUE, link = TRUE, density = 25, square = TRUE, border = TRUE)
{
  ...
  if(screenshot) {
    file_s <- do.call(paste("include", ext, "screenshot", sep = "_"),
      list(file = file, density = density, square = square, border = border))
  }
...
include_png_screenshot <- function(file, out = NULL, density = 25, square = TRUE, border = TRUE)
{
  ## screenshot .png path
  ...
  for(i in seq_along(file)) {
    ...
    cmd <- sprintf("mogrify -resize %s %s %s %s",
      density * 8.268,
      if(square) sprintf("-extent %sx%s", ceiling(density * 8.268), ceiling(density * 8.268)) else "",
      if(border) sprintf("-border %sx%s -bordercolor '#3b3b3b'", ceiling(density/15), ceiling(density/15)) else "",
      out[i])
    system(cmd)    
  }
...
include_pdf_screenshot <- function(file, out = NULL, page = 1, density = 25, square = TRUE, border = TRUE)
{
  ## screenshot .png path
  ...
  for(i in seq_along(file)) {
    cmd <- sprintf("convert -density %s %s[%s] -extent %sx%s %s %s",
      density, file[i], page - 1,
      round(density * 8.268), ceiling(density * if(square) 8.268 else 11.693),
      if(border) sprintf("-border %sx%s -bordercolor '#3b3b3b'", ceiling(density/15), ceiling(density/15)) else "",
      out[i])
    system(cmd)
  }
...
include_html_screenshot <- function(file, out = NULL, density = 25, square = TRUE, border = TRUE)
{
  ## screenshot .png path
  if(is.null(out)) out <- paste0(tools::file_path_sans_ext(basename(file)), ".png")
  if(knitr::opts_chunk$get("fig.path") != "figure/") out <- paste0(knitr::opts_chunk$get("fig.path"), out)
