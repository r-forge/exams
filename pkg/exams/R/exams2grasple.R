...
make_exams_write_grasple <- function(name = NULL, license_name = NULL, license_description = NULL, license_value = NULL, license_link = NULL, zip = TRUE, use_solutionlist = TRUE)
{
  ...
  qtemp <- list(
    ...
    name = "1",
    mode = "at_once",
    license = c(name = if(is.null(license_name)) "Non-explicit license" else license_name,
                  description = if(is.null(license_description)) "No known license for this content item" else license_description,
                  value = if(is.null(license_value)) "non-explicit" else license_value,
                  link = if(is.null(license_link)) "https:\\/\\/en.wikipedia.org\\/wiki\\/Copyright" else gsub("/", "\\\\/", license_link)),
    questions = list(list(
      id = 2,
      type = "open-math",
      question = list(
