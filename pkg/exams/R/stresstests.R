...
stresstest_exercise <- function(file, n = 100, verbose = TRUE, seeds = NULL,
  ...
  if(length(file) > 1L) {
    ...
    xexams_warnings <- rep(NA_character_, n)
    xexams_errors   <- rep(NA_character_, n)
    ...
    ## Append character vectors with fetched warnings and errors if
    ## needed. If no warnings or errors have been fetched, these element
    ## will not be added to the returned object.
    if (!all(is.na(xexams_warnings))) rval$warnings <- xexams_warnings
    if (!all(is.na(xexams_errors)))   rval$errors   <- xexams_errors
    ## Same with 'overall warning and errror count', only added if
    ## at least one warning or error has been fetched.
    if (sum(xexams_we_count) > 0L)    rval$count    <- xexams_we_count
...
## Helper function to combine fetched errors and warnings.
## 'x' is expected to be a character vector of length >= 0,
## If `length(x) == 0L`, NA_character_ is returned.
combine_warnings_errors <- function(x) {
    if (length(x) == 0L) return(NA_character_)
    x <- gsub("\\s{2,}", " ", gsub("\\n", " ", x))
    with(list(z = table(x)), paste(sprintf("%s (%dx)", names(z), z), collapse = "; "))
}
