...
The mean of the response \texttt{y} \Sexpr{direct} with increasing \texttt{x1}. Variable \texttt{x2} has no
influence on the response since the estimated confidence interval contains zero.
...
The R-squared is \Sexpr{signif(rsq, 4)} and thus the variance explained by the regression is \Sexpr{fmt(100 * rsq, 2)}.
