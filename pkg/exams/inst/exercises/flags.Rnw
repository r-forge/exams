...
que <- c(country, err)
ans <- sprintf("%s is the flag of %s.", frmt(que), frmt(que, "name"))
sol <- c(TRUE, rep(FALSE, length(err)))
@


\begin{question}
Which is the flag of \Sexpr{frmt(country, "name")}?

<<questionlist, echo = FALSE, results = tex>>=
answerlist(frmt(que), markup = "latex")
@
\end{question}
