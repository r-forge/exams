...
while(!ok) {
  ...
  ok <- sum(tab) == 1 & all(tab > 0) & all(tab < 1) & ((k %% 1) == 0)
}
...
Starting from
$$
P(E \cap R) = P(E) \cdot P(R | E) = \Sexpr{pe} \cdot \Sexpr{per} = \Sexpr{prob1},
$$
we obtain
$$
P(E \cap \overline{R}) = P(E) \cdot P(\overline{R} | E) = P(E) \cdot (1 - P(R | E)) =
  \Sexpr{pe} \cdot (1 - \Sexpr{per}) = \Sexpr{prob2}.
$$
The remaining probabilities are calculated by:
$$
P(\overline{E} \cap R) = P(\overline{E}) \cdot (1 - P(\overline{R} | \overline{E})) =
  \Sexpr{1 - pe} \cdot (1 - \Sexpr{pnenr}) = \Sexpr{prob3}
$$
and
$$
P(\overline{E} \cap \overline{R}) = P(\overline{E}) \cdot P(\overline{R} | \overline{E}) =
  \Sexpr{1 - pe} \cdot \Sexpr{pnenr} = \Sexpr{prob4}.
$$

\begin{tabular}{|c|cc|c|}
\hline
                 & $~R~$                  & $~\overline{R}~$       & \\ \hline
$~E~$            & ~\bf $\Sexpr{tab2[1, 1]}$~                      & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[1, 2]}$~ & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[1, 3]}$~ \\
$~\overline{E}~$ & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[2, 1]}$~ & ~\bf $\Sexpr{tab2[2, 2]}$~                      & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[2, 3]}$~ \\ \hline
                 & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[3, 1]}$~ & ~\color[rgb]{0.3,0.3,0.3} $\Sexpr{tab2[3, 2]}$~ & ~\bf $\Sexpr{tab2[3, 3]}$~ \\
\hline
\end{tabular}
...
\begin{answerlist}
\item $P(E \cap R) = \Sexpr{prob1 * 100}\%$
\item $P(\overline{E} \cap R) = \Sexpr{prob3 * 100}\%$
\item $P(E \cap \overline{R}) = \Sexpr{prob2 * 100}\%$
\item $P(\overline{E} \cap \overline{R}) = \Sexpr{prob4 * 100}\%$
\item $P(R) = \Sexpr{(prob1 + prob3) * 100}\%$
\item $P(\overline{R}) = \Sexpr{(prob2 + prob4) * 100}\%$
\item $P(E) = \Sexpr{(prob1 + prob2) * 100}\%$
\item $P(\overline{E}) = \Sexpr{(prob3 + prob4) * 100}\%$
\item $P(\Omega) = 100\%$
\end{answerlist}
