...
\title{Flexible Generation of E-Learning Exams and Beyond}
\author{Achim Zeileis}
\URL{http://eeecon.uibk.ac.at/~zeileis/}
\lecture{Flexible Generation of E-Learning Exams and Beyond}
...
\begin{itemize}
  ...
  \item Different types assessments from the same pool of exercises:
    Written exams, online tests, live quizzes, etc.
\end{itemize}
...
\textbf{Challenges:} Based on the same pool of exercises\dots
\begin{itemize}
  \item \textit{Scalable exams:} Automatic generation of a large number
    of different exams.
  \item \textit{Flexible rendering:} Output for written exams or
    online learning management systems, etc.
  \item \textit{Associated self-study materials:} Collections of exercises
    along with solutions.
  \item \textit{Joint development:} Development and maintenance in a
...
\textbf{Exercise templates:} \code{.Rnw} or \code{.Rmd} files composed of
\begin{itemize}
  \item \proglang{R} code chunks for random data generation.
  \begin{itemize}
    \item \code{.Rnw}: Within \verb|<<>>=| and \code{@}.
    \item \code{.Rmd}: Within \verb|```{r}| and \verb|```|.
  \end{itemize}
  \item Question and solution descriptions contained in 
    sections with corresponding names.
  \begin{itemize}
    \item \code{.Rnw}: \verb|\begin|/\verb|\end| pairs for \verb|{question}|/\verb|{solution}|.
    \item \code{.Rmd}: \code{Question}/\code{Solution} sections with \verb|#######| markup.
  \end{itemize}
  \item Metainformation about \code{extype} (numeric, multiple
    choice, \dots), correct \code{exsolution}, a short \code{exname}, etc.
  \begin{itemize}
    \item \code{.Rnw}: \verb|\extype{mchoice}|, \verb|\exsolution{01001}|, \dots
    \item \code{.Rmd}: \verb|extype: mchoice|, \verb|exsolution: 01001|, \dots.
  \end{itemize}
  \item Question and basic metainformation is mandatory -- everything else
    optional. All parts can contain \proglang{R} code chunks or data.
  \begin{itemize}
    \item \code{.Rnw}: \verb|\|\verb|Sexpr{...}|.
    \item \code{.Rmd}: \verb|`r ...`|.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item Links to dynamically generated data can be easily included, e.g.,
    \verb|\url{mydata.rda}| (\code{.Rnw}) or \verb|[mydata.rda](mydata.rda)| (\code{.Rmd}).
\end{itemize}
...
\begin{itemize}
  \item Large-scale courses with \pkg{OpenOLAT} support.
  \item Team of about 5--10 persons (professors, lecturers, student assistants) contribute to the
    pool of exercises.
  \item During the semester, several online tests (and self tests) are carried out in
    \pkg{OpenOLAT} (via \code{exams2qti12}) using numerical and multiple-choice exercises.
  \item Written exams (via \code{exams2nops}) are carried out using single-choice exercises.
    Results are scanned (via \code{nops\_scan}) and automatically evaluated (via \code{nops\_eval}).
    Individual HTML reports are uploaded for each student into \pkg{OpenOLAT}.
\end{itemize}
