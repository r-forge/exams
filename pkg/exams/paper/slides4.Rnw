...
\textbf{Motivation:}
\begin{itemize}
  \item Many statisticians (like myself)
        teach large lecture courses, also as support for other fields.
...
\end{minipage}
\begin{minipage}[t]{0.6\textwidth}
\only<2>{\raisebox{-\height}{\fbox{\includegraphics[width=\textwidth]{slides4-conferences-1}}}}%
...
exams2nops(myexam, n = 3, dir = odir,
  language = "nl", institution = "TEA 2018")
@
