...
\hline
{\bf Statistical computing} & \proglang{R}                                          & Random data generation, computations   \\
{\bf Writing/reporting}     & {\LaTeX}, \emph{Markdown}                             & Text formatting, mathematical notation \\
{\bf Reproducible research} & \pkg{knitr}, \pkg{rmarkdown}, \texttt{Sweave}         & Dynamically tie everything together    \\
{\bf Document conversion}   & \emph{TtH}/\emph{TtM}, \emph{pandoc}                  & Conversion to HTML and beyond          \\
{\bf Image manipulation}    & \emph{ImageMagick}, \pkg{magick}, \pkg{png}           & Embedding graphics		     \\
{\bf Web technologies}      & \pkg{base64enc}, \pkg{RCurl}, \dots                   & Embedding supplementary files	     \\
{\bf Learning management}   & \emph{Moodle}, \emph{OpenOLAT}, \emph{ARSnova}, \dots & E-learning infrastructure 	     \\
\hline
\end{tabular}
}
...
\textbf{Example:} Which of these institutions already hosted a useR! or eRum conference?
...
\verb|Answerlist|\\
\verb|----------|\\
\verb|* Uniwersytet Ekonomiczny w Poznaniu|\\
\verb|* Agrocampus Ouest|\\
\verb|* Technische Universit|{\tt \"a}\verb|t Dortmund|\\
\verb|* Universit|{\tt \"a}\verb|t Wien|\\
\verb|* ETH Z|{\tt \"u}\verb|rich|\\
\verb|* K|{\tt \o}\verb|benhavns Universitet|\\
}
...
\verb|Answerlist|\\
\verb|----------|\\
\verb|* True. eRum 2016 was hosted in Poznan.|\\
\verb|* True. useR! 2009 was hosted at Agrocampus Ouest, Rennes.|\\
\verb|* True. useR! 2008 was hosted at TU Dortmund.|\\
\verb|* False. Universit|{\tt \"a}\verb|t Wien did not host an R conference yet (only TU Wien and WU Wien).|\\
\verb|* False. ETH Z|{\tt \"u}\verb|rich did not host an R conference yet.|\\
\verb|* False. K|{\tt \o}\verb|benhavns Universitet hosted DSC but not useR! or eRum.|\\
}
...
\only<2>{\raisebox{-\height}{\fbox{\includegraphics{slides4-swisscapital-1}}}}%
\only<3>{\raisebox{-\height}{\fbox{\includegraphics{slides4-deriv2-1}}}}
...
<<echo=FALSE, results=hide>>=
set.seed(2018-05-16)
exams2pdf("conferences.Rmd", dir = ".", name = "slides4-conferences-", template = "standalone.tex")
set.seed(2018-05-16)
...
\end{minipage}
\begin{minipage}[t]{0.57\textwidth}
\only<2>{\raisebox{-\height}{\fbox{\includegraphics{slides4-conferences-1}}}}%
\only<3>{\raisebox{-\height}{\fbox{\includegraphics[trim=0 50 0 0,clip]{slides4-boxplots-1}}}}
...
\end{minipage}
\begin{minipage}[t]{0.57\textwidth}
\only<2>{\raisebox{-\height}{\fbox{\includegraphics{slides4-fruit-1}}}}%
...
\end{minipage}
\begin{minipage}[t]{0.57\textwidth}
\only<2>{\raisebox{-\height}{\fbox{\includegraphics{slides4-function-1}}}}%
...
\end{minipage}
\begin{minipage}[t]{0.57\textwidth}
\only<2>{\raisebox{-\height}{\fbox{\includegraphics{slides4-lm-1}}}}%
...
exams2nops(myexam, n = 3, dir = odir,
  language = "hu", institution = "eRum 2018")
@
...
\textbf{Other:} \code{exams2pdf()}, \code{exams2html()}, \code{exams2qti12()}, \code{exams2qti21()}, \dots
...
\begin{frame}[plain]
  ...
  \draw[color=white,fill=white,opacity=0.90] 
  (-10,-8) rectangle (20,-6.6);
  \node at (0,-7.3) [right] {\usebeamercolor[fg]{verbcolor} \fontsize{20}{24} \bfseries\selectfont What Else?};
  \end{tikzpicture}
\end{frame}
\usebackgroundtemplate{}
...
png("whatelse.png", height = 9, width = 12, units = "in", res = 150)
par(mfrow = c(2, 2))
...
\begin{itemize}
  ...
  \item \emph{Stefan Coors, Nikolaus Umlauf:} Graphical exams manager based on \pkg{shiny} that can be
...
\fbox{\includegraphics[width=0.75\textwidth,keepaspectratio=true]{gui-create.png}}
...
\fbox{\includegraphics[width=0.75\textwidth,keepaspectratio=true]{gui-preview.png}}
...
\textbf{Software:} Zeileis A, Gr\"un B, Leisch F, Umlauf N, Birbaumer M, Ernst D, Keller P, Smits N, Stauffer R (2018).
  \textit{\pkg{R/exams}: Automatic Generation of Exams in \proglang{R}.}
  \proglang{R}~package version~2.3-1.
