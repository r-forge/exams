...
\usepackage[bb=boondox,cal=boondoxo]{mathalfa}
...
\begin{itemize}
  ...
  \item In case of problems in the scans, correct with \texttt{nops\_fix()}.
\end{itemize}
\end{minipage}
...
\textbf{5.~Evaluate}
\begin{itemize}
  \item Using \texttt{nops\_eval()}, evaluate the exam to obtain grades, points, etc. and individual HTML reports for each examinee.
...
\begin{tabular}{ll}
  ...
  {\large $\mathbb{X}$}      & \href{https://x.com/AchimZeileis}{@AchimZeileis}
\end{tabular}
