...
\textbf{Olat course:} \url{https://lms.uibk.ac.at/auth/RepositoryEntry/2823520256}
\begin{itemize}
  \item Tutorial for online tests using \code{exams2openolat()}.
  \item Tutorial for written multiple-choice exams using \code{exams2nops()}.
\end{itemize}

\bigskip

\textbf{Gitlab project:} \url{https://git.uibk.ac.at/econstat/exams}
\begin{itemize}
  \item Workflow for large scale multiple-choice exams.
  \item Demo project with R scripts for all steps of the workflow.
\end{itemize}
...
\begin{tabular}{ll}
  ...
  \faIcon[regular]{comments} & \url{https://R-Forge.R-project.org/forum/forum.php?forum_id=4377} \\[0.1cm]
