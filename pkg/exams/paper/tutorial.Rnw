...
\hline
\# & Exercise \phantom{abc} templates              & Dynamic? & Type             & Description \\ \hline
1  & \texttt{expderiv1.Rmd} \texttt{expderiv1.Rnw} & No       & \texttt{num}     & Fixed parameters and numeric solution.\\
2  & \texttt{expderiv2.Rmd} \texttt{expderiv2.Rnw} & No       & \texttt{schoice} & As in \#1 but with single-choice solution (five answer alternatives).\\
3  & \texttt{expderiv3.Rmd} \texttt{expderiv3.Rnw} & Yes      & \texttt{num}     & Randomly-drawn parameters with dynamic computation of correct solution, based on \#1.\\
4  & \texttt{expderiv4.Rmd} \texttt{expderiv4.Rnw} & Yes      & \texttt{schoice} & Randomly-drawn parameters (as in \#3) with dynamically-generated single-choice solution (as in \#2), computed by  \verb|num_to_schoice()|.\\
5  & \texttt{expderiv5.Rmd} \texttt{expderiv5.Rnw} & Yes      & \texttt{schoice} & As in \#4 but with the last alternative: None of the above.\\ \hline
\end{tabular}
}
