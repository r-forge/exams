...
using Moodle, a slightly advanced coding of the \code{.Rnw} is needed.
...
Using Moodle, it is also possible to place answer fields anywhere in the text. This option can
...
\fixme{OLAT/Moodle images and small description of the \code{cloze} examples \dots}
