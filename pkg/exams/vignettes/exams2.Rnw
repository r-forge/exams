...
\code{exams2qti12()} reads the template, replicates the \code{<section>} \code{n} times and
the \code{<item>} for each exercise, and inserts the \verb|##ItemBody| with the
XML transformation functions for \code{num}, \code{mchoice}, etc.
