...
customized. Figure~\ref{fig:exams2moodle-boxplots} shows the first random draw
...
\fixme{comment on import to question bank, add all replications of each exercise as ``random'' questions
into a quiz and customize, solutions}
...
typically did not always allow for modification of the individual exercise item.
...
The main difference \fixme{needs updating} of the generated ZIP file, compared to the \pkg{Moodle} version, is that it just
...
\fixme{comment on where solutions are shown}
...
The idea is that the \code{Sweave} exercises are first \emph{weaved} to {\LaTeX}, 
...
drivers and then call a function \code{xexams()} (for extensible exams) that handles
...
\begin{itemize}
  ...
  \item \code{cloze} for a combination of questions/statements with \code{num},
    ...
    To specify the individual \code{cloze} types\code{clozetype} has to be given, e.g.,
...
temporary directories -- again for each of the user interfaces introduced
...
has not dedicated methods for subsetting etc. However, we refrained from
...
embed raster images of the formulas (e.g., produced through {\LaTeX}). However, this
...
is a function provided by the \pkg{exams} package itself. It proceeds by first runing \code{texi2dvi(pdf = TRUE)}
...
\begin{itemize}
  ...
  \item \code{tex2image} is the ``last resort'' if neither of the two above approaches work.
...
option, \code{base64 = FALSE}, which still needs more detailed explanation. After converting
an exercise from {\LaTeX} to HTML code (using either of the three converters above), the
HTML code may contain \code{<img>} tags with references to supplementary files. Optionally,
...
file is used that is provided within \pkg{exams} and shown in Figure~\ref{fig:exams2html-tstat}. This contains placeholders marked with
\verb|##| that are to be replaced in each randomly drawn exam. The \verb|##id| is simply replaced with a
numeric ID and \verb|##\exinput{exercises}| is replaced by an ordered list (\code{<ol>}) containing
...
\code{exams2moodle()} with \code{make_question_moodle23()}

\code{exams2qti12()} with \code{make_itembody_qti12()}
...
is required (e.g., a different XML format), it might be necessary to develop new drivers for the \pkg{exams}
...
the server to see their personal results and inspect their exam scan.

Fortunately, this exam server (developed by a spin-off company of WU Wien) also
employs an XML specification for importing/exporting its exams. Therefore, it was easily
possible for us to establish a new \code{exams2wu()} interface that produces one ZIP file
for each exam, including the XML plus supplementary graphics. A corresponding \code{write} driver
generator \code{make_exams_write_wu()} is also supplied in the package. Its details are not
discussed here because the XML format adopted is specific to this WU-developed software which
is not widely used. The \code{exams2wu()} interface then essentially proceeds in the following manner:
\begin{Code}
  htmltransform <- make_exercise_transform_html(converter = "tex2image",
    base64 = FALSE)
  wuwrite <- make_exams_write_wu(...)
...
because the WU-developed software does not support MathML. Then, it sets up the custom
...
not least, \fixme{forum at R-Forge?}.
...
the exercise, \code{foo.Rnw} say, works as desired by runing \code{exams2pdf("foo.Rnw")}
...
\code{exams2wu()} interface is employed.
...
\code{confint} & \code{num} & \\
\code{lagrange} & \code{num} & \\
\code{regression} & \code{num} & \\
\code{tstat} & \code{num} & \\ \hline
\code{anova} & \code{mchoice} & \\
\code{boxplots} & \code{mchoice} & \\
\code{cholesky} & \code{mchoice} & \\
\code{relfreq} & \code{mchoice} & \\
\code{scatterplot} & \code{mchoice} & \\
\code{ttest} & \code{mchoice} & \\ \hline
\code{tstat2} & \code{schoice} & \\ \hline
\code{boxhist} & \code{cloze} & \\ \hline
\end{tabular}
