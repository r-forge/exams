...
In some case it is not sufficient to use the arguments of the existing \code{exams2}\emph{xyz}\code{()} functions 
or to provide alternative \code{template}s to them. In particular, when a completely different output format
is required, e.g., a different XML format.


use \code{exams2wu} for illustration?

\code{exams2wu()} essentially proceeds in the following way
\begin{Code}
