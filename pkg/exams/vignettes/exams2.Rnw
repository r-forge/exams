...
core contributors develop \pkg{OpenOLAT} \citep{exams:OpenOLAT:2013}.
...
statistical computing \citep{exams:R:2013} is now available from the
...
odir <- tempfile()
set.seed(1090)
x1 <- exams(myexam, n = 3, dir = odir, template = c("oexam", "osolution"))
@
...
provided by \cite{exams:Gruen+Zeileis:2009}\footnote{A slightly updated version of the
manuscript is also contained as \code{vignette("exams", package = "exams")} within the package}.
...
\citep{exams:MathJax:2013}, an open-source \proglang{JavaScript} engine, can be used to render
...
\end{Code}
\fixme{Describe new \code{rule} argument, link to Appendix~\ref{sec:eval} \dots}
...
\end{Code}
%
\fixme{Describe new \code{eval} argument, link to Appendix~\ref{sec:eval} \dots}
...
the department and had only limited prior experience with \proglang{R} (all had
