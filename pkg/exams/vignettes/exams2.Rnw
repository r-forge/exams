...
\fixme{Point out that Section~\ref{sec:usage} is sufficient for getting an overview
of the package and getting started. But all is needed for full customization etc.}
...
\fixme{Prerequisite: Sweave. Getting started with exams: this document. Details on
{\LaTeX}/PDF exams in Gruen and Zeileis 2009}
...
\fixme{Reference to Section~\ref{sec:moodlecustom} in appendix.}
...
\fixme{Get started with exams.skeleton().
Forward reference to all exercises in Table~\ref{tab:exercises} in appendix.
Recommend to first adapt exercises, then customize layout/formatting.
Especially, first use simple markup that works in all output formats.}
...
\fixme{Point out somewhere that the goal is to write files to disk.}
...
\section[Customization of Moodle exams]{Customization of \pkg{Moodle} exams} \label{sec:moodlecustom}

\fixme{More details on how to exploit Moodle features and work with exams output within Moodle.}
