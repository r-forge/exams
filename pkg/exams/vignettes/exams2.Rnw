...
\makebox[\textwidth]{\hrulefill}
\caption{\label{tab:Rnw} A simple \code{Sweave} exercise: \code{tstat.Rnw}.}
\end{figure}
...
\end{enumerate}
\makebox[\textwidth]{\hrulefill}
\caption{\label{tab:pdf} Display of a \code{tstat} exercise as PDF via \code{exams()} (or \code{exams2pdf()}).}
...
\caption{\label{fig:exams2qti12-boxplots} \pkg{OLAT} screenshot.}
\end{figure}
