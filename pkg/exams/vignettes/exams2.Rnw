...
using \pkg{Moodle}, a slightly advanced coding of the \code{.Rnw} is needed.

First of all, it should be noted that even in cases where a solution \code{answerlist} is not
required, it is recommended to supply one. Even if it is an empty list, e.g., created by
<<>>=
answerlist(rep("", length = 3))
@
The reason is that producing PDF files using \code{exams2pdf()} gives an error if it is not provided
and it is generally advisable to check rendering in multiple formats during the production process.

To ease the set up of the solution meta-information, the recommended work flow is to create a
\code{solutions} \code{list} with length of the given answers in the exercises, i.e., each
\code{list} element holds the correct solution of the corresponding sub-question. Note that the
solutions for choice type answers need to be in binary format, which can be achieved, e.g., by the
convenience function \code{mchoice2string()}. The solution \code{list} can then be parsed into the
solution meta-information tag by
\begin{verbatim}
<<echo=FALSE, results=tex>>=
...
exercises contains 5 answers. The order of the tags corresponds to the order of the supplied
solutions in \verb|\exsolution{}|, hence, the number of answer tags must be equal to the number of
solutions.

\fixme{\pkg{OLAT}/\pkg{Moodle} images and small description of the \code{cloze} examples \dots}
