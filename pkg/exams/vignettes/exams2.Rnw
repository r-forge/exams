...
import and export several other formats), \pkg{OLAT} and \pkg{Blackboard}
...
provided by \cite{exams:Gruen+Zeileis:2009}. An overview of the most important arguments
that are also shared by the version~2 interfaces is given in Table~\ref{tab:args}.
...
\pkg{Moodle} are provided in Section~\ref{sec:boku}.
...
\subsection{Creating the first exam}

\readme{new subsection}
...
and the required templates (e.g., for {\LaTeX}, HTML, or XML) along with
...
exercise type, i.e., \code{num}, \code{schoice}, \code{mchoice}, \code{string}.
To restrict this set of exercises, the \code{type} argument of \code{exams_skeleton()}
can be used, e.g., for setting \code{type = c("num", "schoice")}. In any case,
...
\fixme{Further details on points, marks, and other options.}
...
\begin{Code}
  exams2moodle(file, n = 1L, nsamp = NULL, dir,
    ...
    cloze = NULL, zip = FALSE, rule = NULL, ...)
\end{Code}
\fixme{Describe new \code{rule} argument, link to Appendix~\ref{sec:eevaluation} \dots}
...
\end{Code}
%
\fixme{Describe new \code{eval} argument, link to Appendix~\ref{sec:eevaluation} \dots}
...
system were transfered into Sweave files, the \proglang{Fortran} code
...
the department and had only limited prior experience with R (all had
...
\code{confint}     & \code{num}     & Confidence interval for one-sample mean, i.e., result of length two (for version~1 interface).\\
...
\code{currency8}   & \code{num}     & Exercise in UTF-8 encoding (with Euro and Pound symbol
                                      and German umlaut).\\ 
\code{currency9}   & \code{num}     & Same exercise in Latin-9 (or ISO-8859-15) encoding.\\ \hline
...
\section[Evaluating exams]{Evaluating exams} \label{sec:eevaluation}

\fixme{Describe evaluation tools\dots}
