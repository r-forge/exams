...
Many large-scale exams focus on multiple or single choice exercise because they
...
for a first introduction. As this type of exercises is very flexible, it is both
...
downloaded by the test takers and analyzed. There are six questions: three single
choice questions (about modality, skewness, and outliers) and three numeric questions
about quartiles. In \pkg{Moodle} the display is similar but drow-down menus are used
for the single choice questions.

To set up such a \code{cloze} question the six separation solutions have to be provided,
in this particular random version as
\verb+\exsolution{10|001|10|2.54|3.9|3.72}+, and the corresponding types as
...
which has to consist of $10$ ($= 2 + 3 + 2 + 1 + 1 + 1$) items. The corresponding
\code{answerlist} in the \code{solution} also has to consist of 10~items. However,
empty items are allowed which can be easily set up via \code{answerlist(rep("", 10))}.
...
Another example for a cloze exercise is given by \code{fourfold} and \code{fourfold2}, respectively.
The exercise provides some (conditional) probabilities and asks about the corresponding
joint probabilities in the associated fourfold table. In \code{fourfold} (see upper panel of
Figure~\ref{fig:exams2moodle-fourfold}) the standard layout as a list of question fields
is used while \code{fourfold2} employs the custom \pkg{Moodle} layout (see lower panel of
Figure~\ref{fig:exams2moodle-fourfold}), actually displaying a fourfold table (plus marginal
sums). (Both versions have again been created after \code{set.seed(1090)}.)

\fixme{Trade-off: flexibility vs.\ layout. Inclusion vs.\ omission of answerlist.}
