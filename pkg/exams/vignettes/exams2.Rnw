...
provided by \cite{exams:Gruen+Zeileis:2009}, als provided in an updated version as
...
exercises there is more flexibility: One could either require all
parts of an answer to be exactly correct or one could assign partial
credits.
...
\begin{itemize}
  \item \pkg{Moodle} only provides partial evaluation of multiple-choice
    ...
    uses 1/\#wrong while \code{"false2"} uses $1/\max(\#wrong, 2)$;
