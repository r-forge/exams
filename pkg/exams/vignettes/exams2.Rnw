...
can be displayed in \pkg{OLAT} immediately after completing each
individual exercise as a hint or after the exam as a solution
\fixme{Niki, please check this and also complete the corresponding manual page}.
By default, both are disabled but could be enabled by using the corresponding switch,
see \code{?exams2qti12}.
...
containing all \code{n} replications of all exercises rather than one (or more) files
per exam. The reason is that learning management systems such as \pkg{Moodle} or \pkg{OLAT}
provide their own functionality for randomly drawing questions from a pool of questions
...
\code{xexams()} is processed subsequently and embedded into suitable XML files.
For both functions, an exercise-specific XML transformation generator is used: Because in
\pkg{Moodle} the exercises are called questions, \code{exams2moodle()} internally
employs \code{make_question_moodle23()} whereas \code{exams2qti12()} uses
\code{make_itembody_qti12()} (as exercises are called items in \pkg{OLAT}).



mention MathML display of matrices within OLAT
\code{cholesky} has code for solution
...
not least, the \pkg{exams} package is hosted and \proglang{R}-Forge \citep{exams:Theussl+Zeileis:2009}
...
We are indebted to all our colleagues and mathematics team members at the Department of Statistics
at Universit\"at Innsbruck -- especially Bettina Lubach, Lorenz Oberhammer, and Alexander Razen --
for testing and challenging the code and making suggestions for improvement.
This project was also partially supported by an e-learning grant by the Universit\"at Innsbruck.
\fixme{Does this have a reference/number?}
