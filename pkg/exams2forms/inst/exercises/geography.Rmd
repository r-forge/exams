...
display   <- match.arg(display, c("name", "flag", "combined"))
...
frmt <- function(x, display = "combined") {
  switch(display, "name" = x, "flag" = flag[x], "combined" = paste(x, flag[x]))
}

## question and solution
que <- c(frmt(primary, display), frmt(secondary, display))
ans <- c(sprintf("True. %s is a direct neighbor of %s.", frmt(primary, display), frmt(country, display)),
  sprintf("False. %s does not share a direct border with %s.", frmt(secondary, display), frmt(country, display)))  
sol <- rep(c(TRUE, FALSE), c(length(primary), length(secondary)))
```


Question
========
`r frmt(country, display)` is a country in `r continent`. Which of the following countries are
direct neighbors of `r frmt(country, display)`, i.e., share a border?

```{r questionlist, echo = FALSE, results = "asis"}
answerlist(que, markup = "markdown")
```

Solution
========
The direct neighbors of `r frmt(country)` are: `r paste(frmt(primary), collapse = ", ")`.

The following countries do not share a boundary with `r frmt(country)` but only with one of its neighbors:
`r paste(frmt(secondary), collapse = ", ")`.
...
Meta-information
================
exname: Geography, neighbors of countries
extype: mchoice
