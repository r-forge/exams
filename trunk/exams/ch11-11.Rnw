...
aibegmin  <- 1 + 4               # 1.Jahr nicht (MA-Glättung)  
...
if (j0 == 1) { j0 <- ajahr1} else{ j0 <- ajahr2 }
...
{\bf CH11-11:} Gegeben sind die vierteljährlichen Verkaufszahlen einer 
...
geeignete Glättung, $T_t$, ermittelt. 
...
\vspace{0.3cm}
Geben Sie die Saison bereinigte Reihe an.\\ 
Als Lösung tragen Sie nur 
den Wert des \Sexpr{quartalj} ein.
...
Glättung, $(Y_t - T_t)$, für das fix gewählte Quartal. 
$$S(q) = \frac{1}{J} \sum_{j=1}^J (Y_{j.q} - T_{j.q}) $$
Für das \Sexpr{quartal} ergibt sich
$$S(\Sexpr{q0}) = [( \Sexpr{y[q0]}-\Sexpr{y.t[q0]} ) + 
                   ( \Sexpr{y[q0+4]}-\Sexpr{y.t[q0+4]} ) ] / 2 
   = \Sexpr{ysq0}$$
Die saisonbereinigte Reihe erhält man nach Subtraktion des saisonalen 
