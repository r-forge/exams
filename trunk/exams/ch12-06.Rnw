...
pfc_string <- paste("Prognose: ", round(p_fc, digits = 1),
  " (toleriert: ", round(p_fc, digits = 1) - 0.1, " <= B <= ",
  round(p_fc, digits = 1) + 0.1, ")", sep = "")
...
Berechnen Sie die Prognose des  Dow-Jones-Industrial Index für \Sexpr{tfr_d[3]}.
...
Gesucht ist die 3-Schritt Prognose für den Index, 
$P_{\Sexpr{pend_d}}(3)$.\\

Die $k$-Schritt Prognose für den Kurs einer Aktie ist 
$$P_n(k) = P_n \cdot \exp{\Sigma_{j=1}^k r_n(j) } $$
Die Summe der Prognosen der Renditen 
