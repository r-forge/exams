...
Wien, Vienna, Austria, is attended each semester by 1000 to 1500
business students in their first or second year. Several lecturers
from the Department of Statistics and Mathematics teach this course in
parallel. A re-design of the lecture was undertaken to ensure an
efficient and transparent organisation for lecturers and students. The
primary aims to achieve these goals were (1) to have scalable exams
and (2) to allow a joint development and maintenance of a pool of
exercises and problems for the exams by all lecturers. About 10 to 15
lecturers were involved in contributing to the pool of exercises as
all potential lecturers were participating in the re-design and all
these lecturers are also potential maintainers.
...
already the correct answer of a multiple choice question from a
certain problem description. This signifies that ideally for each
...
package also contains additional material like examplary exercise
...
The underlying motivation for each exercise is an examplary
...
\verb|\|\verb|Sexpr{}| statments which allow to directly input numbers
...
type of question, the solution, some additional informative text as
well as a tolerance for numeric solutions. All the meta-data
specification is inside a comment environment for {\LaTeX} code such
that it is hidden in the final output file. The type of question is
...
text can be included in a string via \verb|\exname{}|. This allows to
provide more information on the type of problem. Finally
\verb|\extol{}| for numeric solutions specifies the allowed deviation
from the solution calculated with \proglang{R} in order to still
assess a given answer as correct. A tolerance for numeric results
allows to account for rounding errors. These tolerances might either
be chosen fixed or can also be defined data set specific. The
...
{\LaTeX} file. The file ``plain.tex'' provided by package \pkg{exams}
...
``exam.tex'' but with the correct answers already filled in.
...
``plain.tex'' is used to generate the PDF file.
...
Wirtschaftsuniversit\"at Wien. The self-studying materials available
