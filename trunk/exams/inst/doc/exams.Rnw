...
either have a single number as result which only need to be checked to
assess the correctness, multiple-choice questions or questions which
require only a short text answer. These requirements on maintenance,
...
\begin{itemize}
  ...
  \item \verb|\exname{}|: optional tolerance limit(s). For numeric solutions
	...
	different tolerences can be supplied for falling below or exceeding
...
Our starting point for the development of the exercise pool was that
business students are more likely to be only consumers of statistics
in their careers and daily lives instead of doing statistical analysis
themselves. Hence, the exercises should not only require students to
do calculations but also cover interpretation of results, statistical
graphics and statistical software output. No restrictions on the
content are imposed by package \pkg{exams}. However, it requires the
exercises to have a certain structure. At WU Wien this helped us to
define a common suitable format for the exercises and their solutions
a-priori and communicate this to the contributing lecturers as well as
the students.

For multiple choice questions it is possible to either ensure that
exactly one option is correct or to allow that all to no options are
correct. We decided on the second variant and students were instructed
accordingly. If the strategy is chosen that only exactly one option is
always correct random answering needs to be prevented by penalizing
incorrect answers and has the drawback that the correct answer can be
guessed by eliminating wrong options. With respect to tolerances for
the numeric solutions we decided to require a certain fixed precision,
i.e., the solutions which are rounded to three digits only need to be
correct for two digits. Other approaches to assess the correctness of
numerical solutions are also feasible with package
\pkg{exams}. Interval arithmetic could for example be used to
determine the interval of suitable solutions. In addition the
tolerances can be set to ensure that only feasible values are within
the tolerance limits, e.g., by using for the one-sample $t$~test
statistic example
\verb|\extol{\|\verb|Sexpr{min(0.01, abs(tstat))}}{0.01}| to ensure to
have only positive values within the tolerance.

Package \pkg{exams} provides the technological framework for the
generation of structured exams. The user does not need to worry about
these details and can focus on the specification of the content and
the development of the pool of exercises. For further information and
guidelines on statistical education and assessment see for example
\cite{exams:Gal+Garfield:1997} and \cite{exams:Garfield+Chance:2000}.
They discuss issues such as topics covered and skills developed in
statistics courses as well as suitable ways of assessment. Large-scale
exams certainly impose special constraints on assessment because of
limited resources. The development of effective multiple choice
questions which force the student to understand underlying statistical
concepts is crucial and often not straight forward. For some hints on
good multiple choice questions especially if they are also used for
self-study materials see for example \cite{exams:Klinke:2004}.
