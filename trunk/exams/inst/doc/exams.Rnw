...
about the exercise such as type and the solution. Meta-information for
...
using either the application specified in the file
associations on a Windows operating system or with the PDF viewer
specified in the options (see \code{getOption("pdfviewer")}) on other
operating systems (e.g., Linux or Mac OS).
...
Sweave for \proglang{R} input and output. For further information about
...
<<>>=
tdir <- tempdir()
sol <- exams(myexam, n = 5, dir = tdir, template = "exam", 
