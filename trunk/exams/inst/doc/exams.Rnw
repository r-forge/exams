...
\item inclusion of a suitable cover page wtih answer fields,
...
@ 
\end{verbatim}
where \code{file} species a list/vector of exercise Sweave files
(see Section~\ref{sec:exercises}), \code{template} is the name of the master
{\LaTeX} file (see Section~\ref{sec:master}), and \code{n} is the number of
random replications. The remaining arguments control details of the
processing, e.g., the directories for input/output files. A technical
manual is available on the help page \code{?exams}, a brief hands-on
introduction to selected arguments is given below:
\begin{itemize}
  \item \code{file}: This is either a character vector containing the
...
@ 
  ...
  \item \code{n}: Number of randomly generated exams.
  
  \item \code{dir}: Output directory for storing the resulting PDF files (and
...
@
  ...
  \item \code{template}: A (vector of) master {\LaTeX} file(s). If more than
	...
	omitted in the file name and the the corresponding files should either
	be in the local directory (or provided with the full path) or in the
	\code{tex} directory of the installed package.
	
  \item \code{header}: A list specifying additional {\LaTeX} commands for
        replacement of \verb|\exinput{header}| in the master {\LaTeX} file.
	It has to be a list of
	\code{list(}\textit{command}\code{ = }\textit{value}\code{)} pairs,
	where \textit{value} can either be a static string or a function
	computing a string from the index $i$ of the $i$the exam. Below we
	use \code{ID = getID} with
<<>>=
getID <- function(i) 
...
@ 
  ...
  \item Further arguments: \code{inputs} can specify a list of files
