...
master {\LaTeX} files controlling the appearance and (3) a
...
files contained one for generating exams, one for the self-studying
...
overview on function \code{exams()} with an exemplary application and
...
suitable answer fields. All meta-data commands are in {\LaTeX} style
but are actually commented out and thus hidden in the final output
file. The Sweave file needs to contain the specification of the type
of question (\verb|\extype{}|), the solution (\verb|\exsolution{}|)
and some additional informative text (\verb|\exname{}|). Optionally a
tolerance (\verb|\extol{}|) can be specified for numeric solutions;
the default is a tolerance of zero.
...
Finally, some meta-data is specified. The type of question is
...
specification(s) of \code{LaTeX} templates. It can be \code{"foo"} or
...
template) are created. Further arguments to \code{exams()} are
...
several exams are written in the course. 
...
The resulting PDF files are stored in \code{tempdir()}. The returned
object consists of a list of length equal to \code{n} and is of class
``\code{\Sexpr{class(sol)}}''. Each element of this list contains
...
for improvement. This research was partially supported by the the
Austrian Science Foundation (FWF) under Hertha-Firnberg grant T351.
