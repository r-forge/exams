...
file. It ensures that all files needed by \code{Sweave()} and
\code{texi2dvi()} to produce the final PDF file are available in a
(temporary) directory. It copies the exercise files as well as
additionally specified input files to this directory and writes to
this directory the final {\LaTeX} file produced by processing the
master {\LaTeX} file and replacing markups with the appropriate
{\LaTeX} code generated with \proglang{R}. 

The master {\LaTeX} file needs to define the \code{exercise} and
\code{solution} environments (possibly hiding solutions) and to
specify where the exercises are included, by inserting the markup
...
\code{TEXINPUTS} path. Then the environments for \code{question} and
\code{solution} are defined. The definition of the \code{solution}
environment given in Table~\ref{tab:master} implies that the solution
is contained in the final PDF file. If the solution should be hidden
the definition of the \code{solution} environment needs to be
modified, e.g., to
\verb|\newenvironment{solution}{\comment}{\endcomment}|. After
starting the document only the markup for inclusion of the exercises
is inserted within an enumerate environment. Function \code{exams()}
replaces the line \verb|%% \exinput{exercises}| by input commands for
each exercise which shall be contained in the final PDF file given by
\verb|\input{|{\it filename}\verb|}|.

Further optional markups in the {\LaTeX} master file are:
\begin{itemize}
\item \verb|\exinput{questionnaire}| to indicate where the answer
  fields for the questions are included in the cover page and
\item \verb|\exinput{header}| for inclusion of definitons of
  additional {\LaTeX} commands.
\end{itemize}

Two slightly more complicated exemplary master {\LaTeX} files where
these additional markups are used are included in the package:
\code{exam.tex} and \code{solution.tex}. \code{exam.tex} can be used
to generate exams including a cover sheet for students to fill in
their names as well as their answers to the
problems. \code{solution.tex} will result in a PDF file containing a
...
\code{exams()} depending on the type of problems in the
...
template the layout of the answer fields is determined as well as if
the correct answer is printed. Please note that this implementation
...
\verb|\begin{document}| and is replaced by \code{exams()} such that in
  ...
  commands are constructed from a named list passed to \code{exams()}
  in the argument \code{header}. For each element of the passed list
  the command is constructed as \verb|\|{\it name}\verb|{|{\it
      object}\verb|}| where {\it name} denotes the name of the list
  element and {\it object} its value.  This allows for example to
  include a specific date information as well as consecutive numbers
  as identifiers for the exams.

  \section[Application and customization: Function
  exams()]{Application and customization: Function
    \code{exams()}} \label{sec:customization}
