...
@ 
  ...
  \item[\normalfont \code{nrep}:] Number of elements sampled from each
        list element of \code{file}. The elements are recycled (if necessary) to match 
        the length of \code{file}. 
...
set.seed(1090)
mynrep <- c(1, 2, 1, 1, 1)
sol <- exams(myexam, n = 5, nrep = mynrep, dir = odir, 
...
(exams(myexam, n = 1, nrep = mynrep, 
       template = "exam", control = mycontrol))
@ 
