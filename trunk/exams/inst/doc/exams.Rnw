...
\title{Automatic Generation of Standardized Exams}
...
\Abstract{ Package \pkg{exams} provides a framework for automatic
  ...
  control commands.  This paper gives an overview on the main design
...
%\VignetteIndexEntry{Automatic Generation of Standardized Exams}
...
account in the process were imposed by the environment (limited
resources, time frame, etc.).

This paper focuses on the technology applied and the implemented
infrastructure. The structure of the tools is driven by the
...
ways of assessment. Large-scale exams certainly impose special
constraints on assessment because of limited resources. The
...
questions especially if they are also used for self-study materials see for
...
\begin{itemize}
  ...
  \item \verb|\extol{|\emph{lower}\verb|}{|\emph{upper}\verb|}|:
        ...
        tolerance is set to the same value. This can be useful for the
        automatic evaluation of numeric solutions if some deviation
...
solution only needs to be correct for two digits when rounding to
three digits (as indicated by a tolerance of 0.01) with the additional
constraint that it is a feasible value, i.e., positive.
...
in their calculators. They should then round their final results to
...
either report exactly the same solution as we derived or are far way
off.
