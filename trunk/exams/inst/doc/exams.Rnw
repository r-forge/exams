...
Given a relatively simple structure of exams (sequence of ``standalone'' exercises
...
in the case of multiple-choice two typicaly strategies are: Either
...
assuring that typical small imprecisions lead to results within
tolerance limits while typcal mistakes are are not. In any case,
