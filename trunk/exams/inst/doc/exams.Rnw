...
\title{Automatic Generation of Standardized Large-Scale Exams}
...
\Abstract{
  ...
  generation of standardized statistical exams. To employ the tools,
...
slides, collections of exercises, exams, etc. 

In the course of the re-design decisions on two different aspects had
to be made. On the one hand the content had to be agreed on depending
on the learning outcomes, i.e.~the topics covered in the lecture as
well as the skills checked in the exams had to be specified. On the
other hand the technological tools had to be selected as well as the
infrastructure provided to support the generation of the
material. Additional constraints which also had to be taken into
account in the process were imposed by the environment (restricted
ressources, time frame, etc.).

This paper focuses on the technology applied and the implemented
infrustructure. The structure of the tools is driven by the
requirements of large-scale exams and can hence be useful in this kind
of setting rather independent of the content.  The main challenges
with respect to the applied technology in this process---that the
\pkg{exams} package tries to address---were:
\begin{itemize}
  \item \textit{Scalable exams:} Automatic generation of a large number
        of different exams in order to have an individual test for each student.
...
order to allow for enough variation and avoid ``learning by heart'':
Students should not be able to guess the correct answer of a multiple
choice question from a certain key word in the problem
...
statistical graphics and statistical software output. However, fast
and easy correction should still be possible based only on a short
result as filled in by the students on the cover sheet of their exams.

Additional insights and guidelines on statistical education and
assessment are for example given in \cite{exams:Gal+Garfield:1997} and
\cite{exams:Garfield+Chance:2000}.  They discuss issues such as topics
covered and skills developed in statistics courses as well as suitable
ways of assessments. Large-scale exams certainly impose special
constraints on assessment because of limited ressources. The
development of effective multiple choice questions which force the
student to understand underlying statistical concepts is crucial and
often not straight forward. For some hints on good multiple choice
questions which are also suitable for self-study materials see for
example \cite{exams:Klinke:2004}.
  ...
  \item \textit{Variation:} Exercises are dynamic documents, containing a
	...
	random samples can be drawn easily.
      \item \textit{Correction:} Solutions for exercises are either
        ...
        text answers (e.g., the name of the appropriate test for a
        given problem).
\end{itemize}
...
\begin{itemize}
  ...
  \item \verb|\exsolution{}|: solution. It contains the numeric
        ...
        may also specify two solutions of type
  ...
  \item \verb|\exname{}|: a name or description. This is some
        ...
        within \proglang{R}.
  \item \verb|\extol{|\emph{lower}\verb|}{|\emph{upper}\verb|}|:
        tolerance limit (optional). For numeric solutions two
        tolerance limits can be specified; by default both are assumed to
        be 0. If only the lower tolerance is specified the upper
        tolerance is set to the same value. This can be useful for
        automatic evaluations of numeric solutions if some deviation
        from the correct answer is allowed (e.g., due to rounding
        errors). 
\end{itemize}
The meta-information is taken from the {\LaTeX} file and hence the
solution as well as the tolerance limits can be dynamically determined
using \verb|\|\verb|Sexpr{}|. In Table~\ref{tab:Rnw} for example the
solution only needs to be correct for up to two digits when rounding
to three digits (as indicated by a tolerance of 0.01) with the
additional constraint that it is a feasible value, i.e., positive.
...
We use a temporary output directory but the reader can easily change
\code{odir} to some local directory.
<<>>=
odir <- tempfile()
@ 

We want a different ID for each exam. So we define a function which is
used to substitute a different \verb|\ID{myexam|\textit{ii}\verb|}|
...
The examples implemented covered the topics of an introductory
statistics course. They were either multiple choice questions or
questions with a numeric result. Students were informed that for
numeric results intermediate results should not be rounded but saved
in their calculators. The final results should then be rounded to
three digits. They were told that small deviations from the correct
solution would still be assessed as correct. Specific rules for
rounding were not discussed. Internally most tolerances were set in a
fixed way to ensure that students have the result correctly up to two
digits. In practice the choice of tolerances (if within reasonable
limits) turned out to be rather irrelevant, because most students do
either report the exactly same solution as we derived or are far way
off.
