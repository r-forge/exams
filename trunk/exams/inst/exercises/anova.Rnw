...
dat <- data.frame(Evaluation = rnorm(sum(n),
  ...
  Occupation = factor(rep(1:4, n),
    labels = c("Pupil", "Student", "Employee", "Retired")))
...
if(runif(1) > 0.5) {
  questions[1] <- paste("The test statistics is smaller than $", f2, "$.", sep = "")
  solutions[1] <- myanova[2,5] < f2
  explanations[1] <- paste("The test statistics is $F = ", format(round(myanova[2,5], digits = 3), nsmall = 3),
    "$ and hence ", ifelse(solutions[1], "", "\\\\textit{not}"), " smaller than $", f2, "$.", sep = "")
} else {
  questions[1] <- paste("The test statistics is larger than $", f2, "$.", sep = "")
  solutions[1] <- myanova[2,5] > f2
  explanations[1] <- paste("The test statistics is $F = ", format(round(myanova[2,5], digits = 3), nsmall = 3), 
...
solutions[2] <- FALSE
explanations[2] <- paste("An analysis of variance (ANOVA) always tests the null hypothesis,",
...
\begin{question}
  \Sexpr{sum(n)} persons were questionned in a survey about the design
  of an advertising campaign. Each respondent was asked to evaluate
  the overall impression of an advertisement on a eleven-point scale
  from 0 (bad) to 10 (good). The evaluations are separately summarized
  with respect to type of occupation of the respondents in
  Figure~\ref{fig:ch05-anova1}.
...
%% SOLUTIONS
\begin{solution}
  In order to be able to answer some of the questions the fraction of
  ...
  given in the \texttt{RSS}-column of the ANOVA-table.  Hence the
