...
questions[1] <- "The locations of both distributions are about the same."
...
explanations[2] <- if (solutions[2])
  "Both Distributions have no observations which deviate more than 1.5 times the interquartile range from the median." else "There are observations which deviate more than 1.5 times the interquartile range from the median."
...
questions[5] <- paste("Distribution ", c("A", "B")[i], " is ", 
                      c("about symmetic", "right-skewed", "left-skewed")[j], ".", sep = "")
SK_i <- SK(list(A, B)[[i]])
