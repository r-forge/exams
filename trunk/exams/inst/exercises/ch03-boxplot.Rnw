...
if(runif(1) > 0.5) {
  ...
  explanations[4] <- paste("$\\\\mathit{QD} = ", iqr,
...
\begin{center}
\footnotesize
<<fig=TRUE, height = 2.5, width = 6, echo=FALSE, eps=FALSE, results=hide>>=
...
%% SOLUTIONS
\begin{solution}
Zur Beantwortung der Fragen ben\"otigen wir den Interquartilsabstand ($\mathit{QD}$)
und den Schiefekoeffizienten ($\mathit{SK}$), beide lassen sich direkt aus dem Boxplot
berechnen:
\begin{eqnarray*}
  \mathit{QD} & = &  Q_{0.75} - Q_{0.25}
