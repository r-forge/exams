...
\begin{question}
Abb.\,\ref{fig:ch03-empvf} zeigt die empirische
...
par(las=1,cex=1.2,mai=c(1,1,0.5,0.5))

plot(c(x[1],x[2]),c(vt[1],vt[1]),type="l",xlim=xl,ylim=yl,ylab="",xlab="",axes=FALSE)
axis(1, ax, as.character(ax),pos=c(0,0))
...
@
\caption{\label{fig:ch03-empvf} Sch\"aden in $1000$ GE}
\end{center}
\end{figure}
...
Aus der Abbbildung kann man $f_n(X\leq \Sexpr{data})$, die relative H\"aufigkeit der Schadensf\"alle, die h\"ochstens $\Sexpr{data}$ betragen, ablesen:
...
lines(c(x[n],xl[2]),c(vt[n],vt[n]),col=2)
lines(c(xl[1],x[1]),c(0,0))
lines(c(min(ax),max(ax)),c(0,0))
lines(c(min(ax),max(ax)),c(1,1))
lines(c(max(ax),max(ax)),c(0,1))
lines(c(data/1000,data/1000,0),c(0,solhoechst,solhoechst),lwd=2,lty=1,col=2)
text(-1,solhoechst,as.character(solhoechst),col=2)
lines(c(0,0.3,0.3,0),c(solhoechst,solhoechst-0.01,solhoechst+0.01,solhoechst),lwd=2,col=2)

@
\caption{\label{fig:ch03-empvf2} Sch\"aden in $1000$ GE}
\end{center}
\end{figure}

Man erh\"alt $f_n(X\leq \Sexpr{data})\approx \Sexpr{solhoechst}$. Der Anteil der Schadensf\"alle, die \Sexpr{str1} $\Sexpr{data}$
GE betragen ist \Sexpr{strloes}. Toleriert: $[\Sexpr{tolu},\Sexpr{tolo} ]$.
