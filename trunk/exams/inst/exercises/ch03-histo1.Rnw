...
\begin{question}
Abbildung~\ref{fig:ch03-histo1} zeigt die Verteilung der Preise von Eigentumswohnungen (pro $m^2$). Welcher Quadrat\-meterpreis
...
%% SOLUTIONS
\begin{solution}
Das gesuchte Quantil ist der Quadratmeterpreis, der mit einer H\"aufigkeit von $\Sexpr{proz}$ Prozent
\"uberschritten wird, d.h.\ der Preis, der von
$$100-\Sexpr{proz}=\Sexpr{prozunter}$$ Prozent unterschritten wird. 

Zuerst berechnet man die relativen H\"aufigkeiten der Intervalle, $$\mbox{H\"aufigkeit}=\mbox{H\"ohe} \times \mbox{Breite}. $$ H\"ohen und Breiten liest man aus dem Histogramm ab.
Zum Beispiel betr\"agt die H\"aufigkeit des 1.~Intervalls $$\Sexpr{hoehen[1]}\times\Sexpr{breiten[1]}=\Sexpr{prob[1]}.$$
...
polygon(c(sol,grenzen[iu+1],grenzen[iu+1],sol,sol),c(0,0,hoehen[iu],hoehen[iu],0),col="lightgray")
if(iu<k){for(i in (iu+1):k)polygon(c(grenzen[i],grenzen[i+1],grenzen[i+1],grenzen[i],grenzen[i]),c(0,0,hoehen[i],hoehen[i],0),col="lightgray")}

for(i in 2:k)
{lines(c(grenzen[i],grenzen[i],grenzen[i+1],grenzen[i+1]),c(0,h[i],h[i],0))
text(grenzen[i-1]*0.6+grenzen[i]*0.4,minh*0.5,as.character(prob[i-1]),col=4)
}
text(grenzen[k]*0.6+grenzen[k+1]*0.4,minh*0.5,as.character(prob[k]),col=4)
lines(c(sol,sol), c(0,hoehen[iu]),lty=3,col=4)
...
Die H\"aufigkeit der ersten $\Sexpr{ius}$ Intervalle ist $\Sexpr{cprob}$, das gesuchte Quantil liegt also im
$\Sexpr{iu}$.~Intervall.
...
Der gesuchte Quadratmeterpreis, der von $\Sexpr{proz}\%$ der Wohnungen \"uberschritten wird,
ist damit $Q=\Sexpr{sol}$ (siehe auch Abbildung~\ref{fig:ch03-histo1b}).
\end{solution}
