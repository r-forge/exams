...
Abbildung~\ref{fig:ch03-histo2} zeigt die Verteilung der Preise von Eigentumswohnungen (pro $m^2$). Welcher Quadrat\-meterpreis
wird von  $\Sexpr{proz}\%$ der Wohnungen unterschritten?
...
@
\caption{\label{fig:ch03-histo2} Quadratmeterpreise von Wohnungen}
\end{center}
\end{figure}
...
for(i in
1:(iu-1))polygon(c(grenzen[i],grenzen[i+1],grenzen[i+1],grenzen[i],grenzen[i]),c(0,0,hoehen[i],hoehen[i],0),col="lightgray")
polygon(c(grenzen[iu],sol,sol,grenzen[iu],grenzen[iu]),c(0,0,hoehen[iu],hoehen[iu],0),col="lightgray")
for(i in 2:k)
{lines(c(grenzen[i],grenzen[i],grenzen[i+1],grenzen[i+1]),c(0,h[i],h[i],0))
...
@
\caption{\label{fig:ch03-histo2b} Quadratmeterpreise von Wohnungen}
\end{center}
\end{figure}
...
Der gesuchte Quadratmeterpreis, der von $\Sexpr{proz}\%$ der Wohnungen unterschritten wird,
ist damit $Q=\Sexpr{sol}$ (siehe auch Abbildung~\ref{fig:ch03-histo2b}).
\end{solution}
