...
n <- 9 + sample(1:6, 4)
dat <- data.frame(
  Bewertung = rnorm(sum(n),
    mean = rep(sample(seq(from = sample(25:55, 1), by = 1, length = sample(15:20, 1)), 4)/10, n),
    sd = rep(sample(70:80, 4)/100, n)),
  Beschaeftigung = factor(rep(1:4, n),
    labels = c("Schueler", "Studenten", "Berufstaetige", "Pensionisten")))
...
fm1 <- lm(Bewertung ~ Beschaeftigung, data = dat)
...
solutions[2] <- FALSE
explanations[2] <- "Eine Varianzanalyse (ANOVA) testet immer die Nullhypothese, da{\\\\ss} alle Mittelwerte gleich sind, gegen die Alternative, da{\\\\ss} sie unterschiedlich sind."
...
questions[4] <- "Es kann nachgewiesen werden, da{\\\\ss} die Bewertung der Testpersonen von ihrer Besch\\\\\"aftigung abh\\\\\"angt."
...
explanations[4] <- paste("Der $p$-Wert ist $", format.pval(myanova[2,6], digits = 3),
  ...
  "nachgewiesen werden, da{\\\\ss} sich die Bewertungen nach Besch\\\\\"aftigung der Testperson unterscheiden.")
...
\Sexpr{sum(n)} Personen befragt. Jede Person soll den Gesamteindruck des Plakats auf einer 
Skala von 0 (schlecht) bis 10 (gut) bewerten. Die Bewertungen, getrennt nach Besch\"aftigung
der Testperson, sind in Abbildung~\ref{fig:ch05-anova} zu sehen.
...
@
\caption{\label{fig:ch05-anova} Parallele Boxplots}
\end{center}
\end{figure}

Um zu analysieren, welchen Einflu{\ss} die Besch\"aftigung auf die Bewertung des Werbeplakats
hat, wurde eine Varianzanalyse durchgef\"uhrt:

<<echo=FALSE, results=verbatim>>=
writeLines(capture.output(myanova)[-c(3, 4)])
@
...
%% SOLUTIONS
\begin{solution}
Zur Beantwortung einiger Fragen mu{\ss} das Bestimmtheitsma{\ss} berechnet werden. Daf\"ur
wird die Fehlerquadratsumme bei nur einen einzigen Gesamtmittelwert ($\mathit{RSS}_0$) sowie 
die Fehlerquadratsumme bei unterschiedlichen Mittelwerten nach Besch\"aftigung ($\mathit{RSS}_1$)
