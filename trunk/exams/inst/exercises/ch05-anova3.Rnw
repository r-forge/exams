...
## DATA GENERATION
n <- 40 + sample(1:12, 4, replace = TRUE)
dat <- data.frame(Wartezeit = rnorm(sum(n),
                    mean = rep(runif(4, min = 3, max = 9), n),
                    sd = rep(runif(4, min = 3, max = 4), n)),
                  Supermarkt = factor(rep(1:4, n),
                    labels = c("Sparag", "Consumo", "Hermes", "Teuro")))
Minimum <- 0
dat$Wartezeit[dat$Wartezeit < Minimum] <- Minimum
...
fm0 <- lm(Wartezeit ~ 1, data = dat)
fm1 <- lm(Wartezeit ~ Supermarkt, data = dat)
...
questions[4] <- "Es kann nachgewiesen werden, dass die Wartezeit vom Supermarkt abh\\\\\"angt."
...
explanations[4] <- paste("Der $p$-Wert ist $", format.pval(myanova[2,6], digits = 3),
  ...
  "nachgewiesen werden, dass sich die Wartezeiten f\\\\\"ur die Superm\\\\\"arkte unterscheiden.")
...
verschiedenem Kassensystem wird verglichen. Die Wartezeiten, getrennt
nach Supermarkt, sind in Abbildung~\ref{fig:ch05-anova3} zu sehen.
...
@
\caption{\label{fig:ch05-anova3} Parallele Boxplots}
\end{center}
\end{figure}

Um zu analysieren, welchen Einfluss der gew\"ahlte Supermarkt auf die
Wartezeit des Kunden an der Kasse hat, wurde eine Varianzanalyse
durchgef\"uhrt:
