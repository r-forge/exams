...
study <- if(paired) paste(
  "Es werden", nobs, "Testpersonen ausgew\\\\\"ahlt. Zun\\\\\"achst zeigt man ihnen",
  product[3], "\\\\emph{mit} Logo und fragt nach einer Bewertung auf einer Skala von 0 (schlecht) bis 10 (gut).",
  "Dann zeigt man ihnen", product[3],
  "\\\\emph{ohne} Logo und fragt nach einer weiteren Bewertung."  
) else paste(
  "Es werden", 2*nobs, "Testpersonen ausgew\\\\\"ahlt. Den ersten", nobs, "zeigt man", product[3],
  "\\\\emph{mit} Logo, den zweiten", nobs, "\\\\emph{ohne} Logo.",
  "Anschlie{\\\\ss}end soll jeder Kunde", product[2], "auf einer Skala von 0 (schlecht) bis 10 (gut) bewerten."
)
...
questions[1] <- paste("Die h\\\\\"oheren Bewertungen erh\\\\\"alt", product[2], "mit Logo.")
...
explanations[1] <- if(paired) paste("Die Differenz der mitteleren Bewertungen betr\"agt",
  ...
  "h\\\\\"oher als 0.") else paste("Die mittlere Bewertung f\\\\\"ur ", product[3],
  " mit Logo betr\\\\\"agt ", round(tt$estimate[1], digits = 3), " und ist damit ",
  ifelse(solutions[1], "", "\\\\textit{nicht}"), " h\\\\\"oher als die mittlere Bewertung ohne Logo von ",
  round(tt$estimate[2], digits = 3), ".", sep = "")
...
vis_text <- c("parallele Boxplots der Bewertungen mit und ohne Logo.",
...
solutions[4] <- TRUE
explanations[4] <- paste("Die Frage ist, ob die mittlere Bewertung f\"ur", product[3],
  "mit Logo dieselbe ist wie ohne Logo.")
...
\begin{question}
Ein Mode-Designer m\"ochte untersuchen, ob sich sein wichtigstes Produkt (ein \Sexpr{product[1]})
besser verkauft, wenn er als Logo ein Bild von einem Reiter auf die Brust stickt.
Deshalb f\"uhrt er folgende Kundenbefragung durch: \Sexpr{study}

Der Mode-Designer stellt nun folgende Frage: Unterscheidet sich die mittlere Kundenbewertung
\"uberhaupt f\"ur \Sexpr{product[3]} mit bzw.\ ohne Logo?
