...
## DATA GENERATION
n <- 20 + sample(1:10, 4) * 3
y <- rnorm(sum(n), mean = rep(sample(seq(from = sample(20:60, 1), by = 1, length = sample(15:25, 1)), 4)/10, n),
    sd = rep(sample(10:80, 4)/100, n))
x <- factor(rep(1:4, n))
...
Skala von 0 (schlecht) bis 10 (gut) bewerten. Die Ergebnisse der Bewertungen
wurden getrennt nach Besch\"aftigung der Testpersonen in folgender Tabelle zusammengefa{\ss}t.

\begin{center}
\begin{tabular}{|c|cccc|c|} \hline
 & Sch\"uler & Studenten & Berufst\"atige & Pensionisten & Gesamt \\ \hline
...
Welcher Anteil der Gesamtstreuung der Bewertungen l\"a{\ss}t sich auf Unterschiede
zwischen den vier verschiedenen Personengruppen zur\"uckf\"uhren?
\end{question}


%% SOLUTIONS
\begin{solution}
Zur Beantwortung der Frage mu{\ss} das Bestimmtheitsma{\ss} berechnet werden. Daf\"ur
