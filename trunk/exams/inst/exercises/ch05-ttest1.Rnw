...
dat <- data.frame(wartezeit = c(A, B),
  supermarkt = factor(rep(1:2, c(nA, nB)), levels = 1:2, labels = c("Sparag", "Consumo")))
...
tt <- t.test(wartezeit ~ supermarkt, data = dat, alternative = sample(c("less", "greater", "two.sided"), 1))

questions[1] <- "Der Absolutbetrag der Teststatistik ist gr{\\\\\"o}{\\\\ss}er als 2."
solutions[1] <- abs(tt$statistic) > 2
...
questions[4] <- "Der Test weist nach, da{\\\\ss} man bei Sparag l\\\\\"anger wartet als bei Consumo."
...
explanations[4] <- if(solutions[4]) paste("Das Testergebnis ist signifikant ($p < 0.05$) und damit wird die Alternative nachgewiesen, da{\\\\ss} die Mittelwertdifferenz",
  ...
  "0 ist. Bei Sparag wartet man im Durchschnitt", round(-diff(tt$estimate), digits = 3),
  "Minuten l\\\\\"anger.") else paste(
  ifelse(tt$alternative != "less", "", 
    "Der Test versucht die Alternative nachzuweisen, da{\\\\ss} die Mittelwertdifferenz kleiner als 0 ist, also da{\\\\ss} man bei Sparag k\\\\\"urzer wartet als bei Consumo."),
  ifelse(tt$p.value < 0.05, "", "Das Testergebnis ist nicht signifikant ($p \\\\ge 0.05$)."),
  ifelse(diff(tt$estimate) < 0, "",
    paste("Bei Sparag wartet man im Durschnitt", round(diff(tt$estimate), digits = 3), "Minuten weniger.")))

questions[5] <- "Der Test weist nach, da{\\\\ss} man bei Sparag k\\\\\"urzer wartet als bei Consumo."
...
explanations[5] <- if(solutions[5]) paste("Das Testergebnis ist signifikant ($p < 0.05$) und damit wird die Alternative nachgewiesen, da{\\\\ss} die Mittelwertdifferenz",
  ...
  "0 ist. Bei Sparag wartet man im Durchschnitt", round(diff(tt$estimate), digits = 3),
  "Minuten weniger.") else paste(
  ifelse(tt$alternative != "greater", "", 
    "Der Test versucht die Alternative nachzuweisen, da{\\\\ss} die Mittelwertdifferenz gr\\\\\"o{\\\\ss}er als 0 ist, also da{\\\\ss} man bei Sparag l\\\\\"anger wartet als bei Consumo."),
  ifelse(tt$p.value < 0.05, "", "Das Testergebnis ist nicht signifikant ($p \\\\ge 0.05$)."),
  ifelse(diff(tt$estimate) > 0, "",
    paste("Bei Sparag wartet man im Durschnitt", round(-diff(tt$estimate), digits = 3), "Minuten l\\\\\"anger.")))
@

\begin{question}
Die Wartezeit (in Minuten) an der Kassa bei zwei Superm\"arkten mit verschiedenem Kassensystem
wir verglichen (siehe Abbildung~\ref{fig:ch05-ttest1}).
...
par(mar = c(4, 2, 1, 1))
plot(wartezeit ~ supermarkt, data = dat)
@
\caption{\label{fig:ch05-ttest1} Parallele Boxplots}
\end{center}
\end{figure}
