...
dat <- dgp()
while(length(unique(dat$y)) < length(dat$y) | any(dat$y < 0)) dat <- dgp()
...
wird verglichen. Die beobachteten Wartezeiten der Kunden sind:

<<echo=FALSE, results=verbatim>>=
names(dat) <- c("Wartezeit", "Supermarkt")
print(dat)
names(dat) <- c("y", "x")
@
