...
## plotting function
rankplot <- function(data, labels = NULL, ranks = FALSE)
{
  omar <- par("mar")
...
Autos gemessen. Die resultierenden Daten sind in Abbildung~\ref{fig:ch06-wilcoxon2}
zu sehen.
...
rankplot(dat)
@
\caption{\label{fig:ch06-wilcoxon2} Beobachtete Geschwindigkeiten vor und nach Regel\"anderung.}
\end{center}
\end{figure}
...
%% SOLUTIONS
\begin{solution}
Auf Basis der Visualisierung in Abbildung~\ref{fig:ch06-wilcoxon2} k\"onnen R\"ange
f\"ur die beobachteten Geschwindigkeiten (innerhalb der Gesamtstichprobe) vergeben
werden (siehe Abbildung~\ref{fig:ch06-wilcoxon2a}).
...
\footnotesize
<<fig=TRUE, height=3, width=5.5>>=
rankplot(dat, ranks = TRUE)
@
\caption{\label{fig:ch06-wilcoxon2a} Beobachtete Geschwindigkeiten mit R\"angen.}
\end{center}
\end{figure}
