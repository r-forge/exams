...
ausgew\"ahlten Stunde sind in Abbildung~\ref{fig:ch06-wilcoxplot3} zu sehen.
...
exam_rankplot(dat, yfac = 0.45)
@
\caption{\label{fig:ch06-wilcoxplot3} Beobachtete Ausgaben in den zwei Kundengruppen.}
\end{center}
\end{figure}
...
%% SOLUTIONS
\begin{solution}
Auf Basis der Visualisierung in Abbildung~\ref{fig:ch06-wilcoxplot3} k\"onnen R\"ange
f\"ur die beobachteten Ausgaben (innerhalb der Gesamtstichprobe) vergeben
werden (siehe Abbildung~\ref{fig:ch06-wilcoxplot3a}).
...
\footnotesize
<<fig=TRUE, height=3, width=5.5, echo=FALSE, eps=FALSE, results=hide>>=
exam_rankplot(dat, ranks = TRUE, yfac = 0.45)
@
\caption{\label{fig:ch06-wilcoxplot3a} Beobachtete Ausgaben mit R\"angen.}
\end{center}
\end{figure}
