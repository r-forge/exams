...
if(runif(1) > 0.5) {
  questions[1] <- paste("In der Stichprobe ist der Anteil von Personen mit ", c("erh\\\\\"ohtem", "geringem")[variant], 
  " Risiko bei Frauen h\\\\\"oher als bei M\\\\\"annern.") 
  solutions[1] <- diff(anteil[variant,]) < 0
} else {
  questions[1] <- paste("In der Stichprobe ist der Anteil von Personen mit ", c("erh\\\\\"ohtem", "geringem")[variant], 
  " Risiko bei Frauen niedriger als bei M\\\\\"annern.")
  solutions[1] <- diff(anteil[variant,]) > 0
}
...
Ein Chi-Quadrat-Test zur \"Uberpr\"ufung von Unterschieden im
