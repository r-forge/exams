...
if(runif(1) > 0.5) {
  questions[1] <- paste("Der Anteil der Problemkredite ist in Branche ",
...
explanations[2] <- paste("Bei einer $2 \\\\times ", cols, "$-Tafel ist die Anzahl der Freiheitsgrade gleich $df = (2-1)\\\\cdot(", cols, "-1) = ", 
                   cols-1, "$.", sep = "")
...
gr\"oberen Problemen (mehrfach versp\"atete Zahlungen, keine Zahlungen, usw.) gekommen ist. 
...
Ein $\chi^2$-Test zur \"Uberpr\"ufung von Unterschieden in der Rate 
