...
if(runif(1) > 0.5) {
  ...
  anteil <- sample(seq(10, 30, by = 0.1), 1)
  questions[1] <- paste("Weniger als ", anteil, "\\\\% der Befragten hat die Parteipr\\\\\"aferenz ge\\\\\"andert.", sep = "")
  solutions[1] <- (pabba < anteil/100)
...
explanations[4] <- paste("Der Absolutbetrag der Teststatistik ist gleich ",
                         ...
                         ifelse(abs(T) > 1.96, "verworfen.", "beibehalten."),
                         ifelse(abs(T) > 1.96, paste("Es gibt", ifelse(ab > ba,  "", "\\\\emph{nicht} "), 
...
F\"ur das Berechnen der Teststatistik wird die Anzahl der Befragten
ben\"otigt, die ihre Parteipr\"aferenz ge\"andert haben. Das
entspricht der Anzahl der Beobachtungen in der Gegendiagonale: $n =
...
relative H\"aufigkeit der \"Anderung von Partei A zu B betr\"agt
$\Sexpr{fn.berechnung}$.
