...
if(runif(1) > 0.5) {
  ...
  explanations[1] <- paste ("Die Werte der Gegendiagonale der Kreuztabelle (",
                            tvduell[1,2], " und ", tvduell[2,1],") geben dar\\\\\"uber Auskunft.")
} else {
...
if(abs(z) < 1.96) {
  ...
  if(ab>ba) {
    solutions[4] <- ( ab > ba )
    explanations[4] <- paste("Der Wert der Teststatistik betr\\\\\"agt ", round(z,digits=3), 
                        ". Die Nullhypothese von gleich viel \\\\\"Anderungen in jede Richtung 
                        wird verworfen. In der Stichprobe gibt es mehr \\\\\"Anderungen von A zu B,
                        daher ist diese einseitige Interpretation zul\\\\\"assig.")
} else { 
    solutions[4] <- ( ab > ba )
    explanations[4] <- paste("Der Wert der Teststatistik betr\\\\\"agt ", round(z,digits=3), 
                        ". Die Nullhypothese von gleich viel \\\\\"Anderungen in jede Richtung 
                        wird verworfen. In der Stichprobe gibt es aber mehr \\\\\"Anderungen von B zu A, 
                        daher ist diese einseitige Interpretation nicht zul\\\\\"assig.")       
}
}
...
if(runif(1) > 0.5) {
  ...
  explanations[5] <- "Die 1.Spalte der Kreuztabelle gibt die Pr\\\\\"aferenzen nach den TV-Duellen 
                      f\\\\\"ur Partei A an."
} 
