...
## 1. X2 ablesen
if(runif(1) > 0.5) {
  questions[1] <- paste("Der $X^2$-Wert betr\\\\\"agt:", round(x2,digits=3))
  solutions[1] <- T
  explanations[1] <- "Diesen Wert kann man aus dem \\\\textsf{R}-Output ablesen."
} else {
  questions[1] <- paste("Der $X^2$-Wert betr\\\\\"agt:", round(pv, digits=4) )
  solutions[1] <- F
  explanations[1] <- paste("Der richtige Wert ist aus dem \\\\textsf{R}-Output ablesbar:", round(x2,digits=3))
}
...
if(runif(1) > 0.5) {
  ...
  solutions[2] <- T
  explanations[2] <- "df $= (3-1)\\\\cdot(3-1) = 4$. Das ist auch dem \\\\textsf{R}-Output zu entnehmen."
} else {
