...
Freizeit <- matrix(absh, nrow=3, 
                   dimnames =  list(Sport = c("nie","manchmal","oft"), 
...
explanations[5] <- paste("Der $p$-Wert ist ", format.pval(pv,digits=3), " und damit ", ifelse(pv < 0.05, "", " \\\\emph{nicht} "),
    "kleiner als 0.05. Daher wird die Nullhypothese", ifelse(pv < 0.05, "verworfen.", "beibehalten."))
...
Ein $\chi^2$-Test zur \"Uberpr\"ufung, ob das Aus\"uben der 
