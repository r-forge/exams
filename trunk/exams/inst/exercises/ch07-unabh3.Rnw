...
if(runif(1) > 0.5) {
  questions[1] <- "Es gibt mehr Befragte, die beide Interessen oft verfolgen, 
...
## 2. df
explanations[2] <- "Bei einer $3 \\\\times 3$-Tafel ist die Anzahl der Freiheitsgrade gleich $df = (3-1)\\\\cdot(3-1) = 4$."
if(runif(1) > 0.5) {
