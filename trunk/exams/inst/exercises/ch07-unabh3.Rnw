...
if(runif(1) > 0.5) {
  ...
  explanations[1] <- "Das geben die entsprechenden Eintragungen im linken oberen und rechten 
                      unteren Eck der Kontingenztafel an."
} else {
  questions[1] <- "Unter denen, die nie Sportinteressen verfolgen, gibt es mehr, die oft Kulturinteressen 
                   verfolgen, als solche, die höchstens manchmal Kulturinteressen verfolgen."
  solutions[1] <- Freizeit[3,1] > (Freizeit[3,2]+Freizeit[3,3])
  explanations[1] <- "Das kann man aus der 3.Zeile der Kontingenztafel ableiten."
}
...
explanations[4] <- paste("Der $X^2$-Wert ist ", round(x2, digits=2), 
  ...
  wird die Nullhypothese (Unabh\\\\\"angigkeit) verworfen, sonst beibehalten.", sep = "")
