...
if(runif(1) > 0.5) {
  questions[1] <- "In der Stichprobe gibt es mehr Raucher als Kaffetrinker."
  solutions[1] <- sum(kaffeenikotin[,1]) > sum(kaffeenikotin[,1])
  explanations[1] <- "Vergleich der Werte aus der Kontingenztafel: 
                      1.Spaltensumme verglichen mit 1.Zeilensumme."
} else {
  questions[1] <- "In der Stichprobe gibt es weniger Nichtraucher als Kaffetrinker."
  solutions[1] <- sum(kaffeenikotin[,2]) < sum(kaffeenikotin[,1])
  explanations[1] <- "Vergleich der Werte aus der Kontingenztafel: 
                      2.Spaltensumme verglichen mit 1.Zeilensumme."
}
...
". Ist dieser gr\\\\\"o{\\\\ss}er als 3.84, wird die Nullhypothese (Unabh\\\\\"angigkeit) verworfen, sonst beibehalten.", sep = "")
