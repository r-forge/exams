...
if(runif(1) > 0.5) {
  questions[1] <- paste("Die Mittelwertstabelle passt zu Plot ", dasm, ".")
  solutions[1] <- TRUE
  explanations[1] <- expl[dasm]
} else {
  questions[1] <- paste("Die Mittelwertstabelle passt zu Plot ", ndasm1, ".")
  solutions[1] <- FALSE
  explanations[1] <- expl[ndasm1]
}


## 2. einen falschen Plot identifizieren
 if(runif(1) > 0.5) {
  questions[2] <- paste("Die Mittelwertstabelle passt zu Plot ", ndasm2, ".")
  solutions[2] <- FALSE
  explanations[2] <- expl[ndasm2]
} else {
  questions[2] <- paste("Die Mittelwertstabelle passt zu Plot ", ndasm3, ".")
  solutions[2] <- FALSE
  explanations[2] <- expl[ndasm3]
}
...
if(runif(1) > 0.5) {
  questions[3] <- paste("Die Mittelwertstabelle passt zum Modell ", dasmt, ".")
  solutions[3] <- TRUE
  explanations[3] <- paste( "Richtig. ", expl[dasm] )
} else {
  questions[3] <- paste("Die Mittelwertstabelle passt zum Modell ", ndasmt1, ".")
  solutions[3] <- FALSE
  explanations[3] <- paste( expl[dasm], ", daher ist das richtige Modell: ", dasmt)
}


## 4. ein falsches Modell identifizieren
 if(runif(1) > 0.5) {
  questions[4] <- paste("Die Mittelwertstabelle passt zum Modell ", ndasmt2, ".")
  solutions[4] <- FALSE
  explanations[4] <- paste( expl[dasm], ", daher ist das richtige Modell: ", dasmt)
} else {
  questions[4] <- paste("Die Mittelwertstabelle passt zum Modell ", ndasmt3, ".")
  solutions[4] <- FALSE
  explanations[4] <- paste( expl[dasm], ", daher ist das richtige Modell: ", dasmt)
}
...
expl[2] <- "Von Stufe B1 zu Stufe B2 von Faktor B gibt es einen starken Anstieg 
                   in den Mittelwerten. Bei Faktor A ist der Anstieg nur schwach."
expl[3] <- "Sowohl von Stufe A1 zu Stufe A2 von Faktor A 
   ´        als auch von Stufe B1 zu Stufe B2 von Faktor B gibt es einen starken Anstieg
                   in den Mittelwerten."                   
expl[4] <- "In Stufe B1 ist der Unterschied in den Mittelwerten zwischen den Faktorstufen A1 und A2 
            positiv, in Stufe B2 ist dieser Unterschied negativ."    
...
\begin{question}
Eine Untersuchung mit einer abhängigen metrischen Variablen Y und zwei Faktoren A und B 
...
<<echo=FALSE,fig=TRUE>>= 
par( mfrow = c(2,2) )
interaction.plot( A,B,y1, ylab="Mittelwerte", main="Plot 1", ylim=c(10,50))
interaction.plot( A,B,y2, ylab="Mittelwerte", main="Plot 2", ylim=c(10,50))
interaction.plot( A,B,y3, ylab="Mittelwerte", main="Plot 3", ylim=c(10,50))
interaction.plot( A,B,y4, ylab="Mittelwerte", main="Plot 4", ylim=c(10,50))
##legend(30,10,B)
@
