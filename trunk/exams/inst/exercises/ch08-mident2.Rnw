...
expl[2] <- "Die Aufnahme von Faktor B ergibt einen signifikanten Abfall in der RSS, 
            die weitere Aufnahme von Modellteilen nicht mehr."
expl[3] <- "Die Aufnahme beider Haupteffekte A und B f\\\\\"uhrt zu einem 
...
if(runif(1) > 0.5) {
  ...
  solutions[2] <- FALSE
  explanations[2] <- paste("Das vorgeschlagene Modell bedeutet: ", expl[ndasm1], " Jedoch gilt: ", expl[dasm], sep = "")
}
...
quest[4] <- "Das passende Modell lautet: die Unterschiede in den durchschnittlichen Geh\\\\\"altern 
...
if(runif(1) > 0.5) {
  ...
  solutions[3] <- FALSE
  explanations[3] <- paste("Das vorgeschlagene Modell bedeutet: ", expl[ndasm1], " Jedoch gilt: ", expl[dasm], sep = "")
}
...
if(runif(1) > 0.5) {
  ...
  solutions[4] <- FALSE
  explanations[4] <- paste("Das vorgeschlagene Modell bedeutet: ", expl[ndasm2], " Jedoch gilt: ", expl[dasm], sep = "")
} else {
  questions[4] <- quest[ndasm3]
  solutions[4] <- FALSE
  explanations[4] <- paste("Das vorgeschlagene Modell bedeutet: ", expl[ndasm3], " Jedoch gilt: ", expl[dasm], sep = "")
}
...
der Faktor A (Universit\"at) und wenn einmal zuerst der Faktor B (Branche) 
...
Die Durchschnittseinkommen (berechnet aus jeweils 5 Werten) f\"ur 
