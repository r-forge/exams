...
expl <- character(4)
expl[1] <- "Sowohl der Wechselwirkungseffekt als auch der Haupteffekt Branche k\\\\\"onnen 
            elimiert werden, nicht jedoch Haupteffekt Uni."
expl[2] <- "Sowohl der Wechselwirkungseffekt als auch der Haupteffekt Uni k\\\\\"onnen 
            elimiert werden, nicht jedoch Haupteffekt Branche."
...
expl[4] <- paste("In Stufe B1 ist der Unterschied in den Mittelwerten zwischen den Faktorstufen U1 und U2 ", 
...
expl[4] <- paste( "Bei Stufe B1 ist der Unterschiede in den Mittelwerten zwischen 
                   den Faktorstufen U1 und U2 ", posneg, 
                   " in Stufe B2 ist dieser Unterschied ", negpos, ".", sep = "")    
...
if(runif(1) > 0.5) {
  questions[5] <- paste( "Von Stufe U1 zu Stufe U2 von Faktor Uni gibt es einen starken ", aufab, 
                         " in den Mittelwerten. Bei Faktor Branche ist das nicht der Fall.")
  solutions[5] <- dasm==1
  explanations[5] <- expl[dasm]
} else {
  questions[5] <- paste( "Von Stufe B1 zu Stufe B2 von Faktor Branche gibt es einen starken ", aufab,
                         " in den Mittelwerten. Bei Faktor Uni ist das nicht der Fall.")
  solutions[5] <- dasm==2
  explanations[5] <- expl[dasm]
}
} else {
...
Zwei Branchen (Faktor Branche), in denen die ManagerInnen t\"atig sind, 
und zwei Universit\"aten (Faktor Uni), 
deren AbsolventInnen die ManagerInnen sind, 
werden verglichen.

Als Methode kommt eine zweifache Varianzanalyse zum Einsatz.

Aus dem folgende Modellbaum k\"onnen die p-Werte von mehreren 
Modellvergleichen abgelesen werden.

\setkeys{Gin}{width=0.98\textwidth}
<<echo=FALSE,fig=TRUE,height=7,width=8>>=
selectplot(LM[[5]], LM[[3]], LM[[4]], LM[[2]], LM[[1]],RSS=FALSE,pvalue=TRUE,digits=4)
@

Zus\"atzlich sind die Durchschnittseinkommen f\"ur jede Kombination 
