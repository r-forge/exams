...
a9 <- paste(c("Für X = ", as.character(xh), " ist ungefähr Y = ", as.character(yhr), " zu erwarten."), collapse="")
a10 <- paste(c("Für X = ", as.character(xh), " ist ungefähr Y = ", as.character(yhf), " zu erwarten."), collapse="")
...
if(i1 < 2)
  ...
  {v <- c(FALSE,
         ...
         TRUE,
         abs(yhf-yhr) < 0.01*sy)
  ex[1] <- "Ein standardisiertes Streudigramm liegt nicht vor, weil X und Y nicht beide  
  das Mittel 0 und die Varianz 1 haben."
  }  
   
ex[2] <- if(abs(b - 1) < 0.1) "Die Steigung der Regressionsgeraden
           berechnet sich aus
           $r  s_y/s_x$ und ist in etwa 1." else
           "Die Steigung der Regressionsgeraden berechnet sich aus
           $r s_y/s_x$ und ist daher ungleich 1."
           
ex[3] <- if(ir==1 || ir==4) "Man sieht einen deutlichen Zusammenhang im Streudiagramm. Der
            Betrag des Korrelationskoeffizienten ist nahe bei 1 und daher gr\\\\\"o{\\\\ss}er 0.8." else if(ir==2) "Es ist 
            im Streudiagramm keine Koppelung zu erkennen. Daher ist der
            Korrelationskoeffizient nahe 0." else 
...
ex[9] <- paste("Aus der Regressionsgeraden erh\\\\\"alt man f\\\\\"ur Y an der Stelle ", xh, 
...
\begin{question}
Abbildung~\ref{fig:ch09-streudiag} zeigt ein Streudiagramm. 
...
axis(2,at=yt)
@
\caption{\label{fig:ch09-streudiag} Streudiagramm}
\end{center}
\end{figure}
