...
id1 <- sample(2,1)
qu11 <- c("\\\\beta_1 X_1", "\\\\beta_2 X_2")
id11 <- sample(2,1)
questions[1] <- paste("Beim Testen der Nullhypothese 
    $H_0: Y = \\\\beta_0 + ", qu11[id11], " + \\\\epsilon$ 
    gegen die Alternative $H_A: Y = \\\\beta_0 + \\\\beta_1 X_{1} + \\\\beta_2 X_{2} + \\\\epsilon$ 
    wird die Nullhypothese ", qu1[id1], " (Signifikanzniveau $5\\\\%$).", sep = "")
explanations[1] <- "Mit der vorliegenden ANOVA-Tabelle wird das Testproblem $H_0: Y = \\\\beta_0 + \\\\epsilon$ 
      gegen die Alternative $H_A: Y = \\\\beta_0 + \\\\beta_1 X_{1} + \\\\beta_2 X_{2} + \\\\epsilon$ getestet,
	nicht aber ein einzelner Parameter alleine."
...
explanations[3] <- paste("Das Bestimmtheitsma{\\\\ss} wird berechnet aus $R^2 = 1-RSS_{M_{12}}/RSS_{M_0}$ = ",
...
explanations[4] <- paste("Das Bestimmtheitsma{\\\\ss} wird berechnet aus $R^2 = 1-RSS_{M_{12}}/RSS_{M_0}$ = ",
	round(summary(fm23)$r.squared,2), ".",sep="")  
...
questions[5] <- paste("Beim Testen der Nullhypothese $H_0: Y = \\\\beta_0 + \\\\epsilon$ 
    gegen die Alternative $H_A: Y = \\\\beta_0 + \\\\beta_1 X_{1} + \\\\beta_2 X_{2} + \\\\epsilon$ 
...
questions[6] <- paste("Bei der Wahl eines Modells soll mindestens eine der beiden Variablen 
	$X_1, X_2$ im Modell sein. (Signifikanzniveau $5\\\\%$).", sep = "")
...
explanations[6] <- "Wird die Nullhypothese verworfen, so soll mindestens eine der beiden Variablen 
	$X_1, X_2$ im Modell bleiben. Das ist dann der Fall, wenn der $p$-Wert kleiner als 0.05 ist."  
...
Im Folgenden sehen Sie eine ANOVA zum Testen von $M_0: Y = \beta_0 + \epsilon$ gegen \\
$M_{12}: Y = \beta_0 + \beta_1 X_{1}
+ \beta_2 X_{2} + \epsilon$: 
