...
explanations[1] <- paste("Denn der $p$-Wert ist ungef\\\\\"ahr ", format(round(anova(fm1,fm2)[2,6],4),scientific=FALSE), ".",seps="")  
...
explanations[2] <- paste("Denn der $p$-Wert ist ungef\\\\\"ahr ", format(round(anova(fm3,fm23)[2,6],4),scientific=FALSE), ".",seps="")  
...
explanations[5] <- paste("Die Nullhypothese wird verworfen, wenn
	...
	Der $p$-Wert ist ungef\\\\\"ahr ", format(round(anova(fm2,fm23)[2,6],4),scientific=FALSE), ".",seps="")  
...
Im folgenden sehen Sie die den Baum zur Modellwahl, der auf Modellvergleichen mittels der 
ANOVA basiert: 
...
@
\caption{\label{fig:ch10-tree} Baum zur Modellwahl}
\end{center}
\end{figure}
