...
id1 <- sample(2,1)
questions[1] <- paste("Beim Testen der Nullhypothese $H_0: \\\\texttt{Ausgaben} = \\\\beta_0 + \\\\epsilon$ gegen die Alternative
    $H_1: \\\\texttt{Ausgaben} = \\\\beta_0 + \\\\beta_1 \\\\texttt{Alter} + \\\\epsilon$ wird die Nullhypothese ",
    qu1[id1], " (Signifikanzniveau $5\\\\%$).",sep = "")
solutions[1] <- ((anova(fm1,fm2)[2,6] < 0.05) & (id1 == 1)) || ((anova(fm1,fm2)[2,6] >= 0.05) & (id1 == 2)) 
explanations[1] <- paste("Denn der $p$-Wert ist ", format.pval(anova(fm1,fm2)[2,6],digits=3), ".", sep="")  

id2 <- sample(2,1)
questions[2] <- paste("Beim Testen der Nullhypothese $H_0: \\\\texttt{Ausgaben} = \\\\beta_0 + \\\\beta_2 \\\\texttt{Einkommen} + \\\\epsilon$ 
...
explanations[2] <- paste("Denn der $p$-Wert ist ", format.pval(anova(fm3,fm23)[2,6],digits=3), ".", sep="")  

questions[3] <- paste("Von den 4 zur Verf\\\\\"ugung stehenden Modellen wird das Modell $M_0$
    gew\\\\\"ahlt (Signifikanzniveau $5\\\\%$).", sep = "") 
...
explanations[4] <- if(length(fm0$coef)==3) "Im Baum liefern die oberen beiden Modellvergleiche von $M_1$ gegen $M_{12}$ 
			bzw. $M_2$ gegen $M_{12}$ jeweils signifikante Testgr\\\\\"o{\\\\ss}en. Daher
			werden jeweils die Nullhypothesen verworfen und es wird $M_{12}$ gew\\\\\"ahlt." else
			"Im Baum liefern die oberen beiden Modellvergleiche von $M_1$ gegen $M_{12}$ 
			bzw. $M_2$ gegen $M_{12}$ nicht beide signifikante Testgr\\\\\"o{\\\\ss}en. 
			Daher wird $M_{12}$ nicht gew\\\\\"ahlt."
...
questions[5] <- paste("Beim Testen der Nullhypothese 
    $H_0: \\\\texttt{Ausgaben} = \\\\beta_0 + \\\\beta_1 \\\\texttt{Alter} + \\\\epsilon$ 
    gegen die Alternative $H_1: \\\\texttt{Ausgaben} = \\\\beta_0 + \\\\beta_1 \\\\texttt{Alter} + \\\\beta_2 \\\\texttt{Einkommen} + \\\\epsilon$ 
...
explanations[5] <- paste("Die Nullhypothese wird verworfen, wenn
	der $p$-Wert kleiner als $0.05$ ist.
	Der $p$-Wert ist ", format.pval(anova(fm2,fm23)[2,6],digits=3), ".", sep="")  
...
der Leute abh\"angen. Es stehen die folgenden 4 Modelle zur Auswahl:
\begin{eqnarray*}
& M_0: & \texttt{Ausgaben}   =   \beta_0 + \epsilon \\
& M_1: & \texttt{Ausgaben}   =   \beta_0 + \beta_1 \texttt{Alter} + \epsilon \\
& M_2: & \texttt{Ausgaben}   =   \beta_0 + \beta_2 \texttt{Einkommen} + \epsilon  \\
& M_{12}: & \texttt{Ausgaben}  =  \beta_0 + \beta_1 \texttt{Alter} + \beta_2 \texttt{Einkommen} +
\epsilon 
\end{eqnarray*}

In Abbildung~\ref{fig:ch10-tree2} sehen Sie den Baum zur Modellwahl, der auf Modellvergleichen mittels der 
ANOVA basiert.
...
selectplot(M0,M1,M2,M12,name=TRUE,formula=FALSE,RSS=TRUE,pvalue=TRUE)
@
\caption{\label{fig:ch10-tree2} Baum zur Modellwahl}
\end{center}
\end{figure}
