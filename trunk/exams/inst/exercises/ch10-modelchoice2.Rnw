...
while(!ok) {
  ...
  Einkommen <- x2
  fm1 <- lm(Ausgaben ~ 1)
  fm2 <- lm(Ausgaben ~ Alter)
  fm3 <- lm(Ausgaben ~ Einkommen)
  fm23 <- lm(Ausgaben ~ Alter + Einkommen)
}
...
explanations[1] <- paste("Dieser Test wird in der 3.~Tabelle durchgef\\\\\"uhrt. 
	Dort ist der $p$-Wert ", format.pval(anova(fm1,fm2)[2,6],digits=3), ".",sep="")  
...
explanations[2] <- paste("Dieser Test wird in der 2.~Tabelle durchgef\\\\\"uhrt. 
	Dort ist der $p$-Wert ", format.pval(anova(fm3,fm23)[2,6],digits=3), ".",sep="")  
