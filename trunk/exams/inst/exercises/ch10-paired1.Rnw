...
prog[3] <- sum(fmod$coefficients)
prog[4] <- fmod$coefficients[1]
...
explanations[1] <- if(paired) paste("Die Differenz zwischen den durchschnittlichen Bewertungen mit und ohne 
  Logo betr\\\\\"agt ", round(tt_paired$estimate,4), ". Die durchschnittliche Bewertung f\\\\\"ur das Produkt 
  ...
  Wert f\\\\\"ur das Parfum ausrechnen: ", round(fmod$coefficients[1],4), " + (", round(fmod$coefficients[2],4), ") $\\\\cdot$ (", 
