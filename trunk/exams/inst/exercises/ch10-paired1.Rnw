...
study <- if(paired) paste("Ein Mode-Designer m\\\\\"ochte untersuchen, ob sich sein wichtigstes Produkt 
  ...
  Unterscheidet sich die mittlere Kundenbewertung \\\\\"uberhaupt f\\\\\"ur ", product[3], " mit bzw. ohne Logo?",seps="") else
  ...
  f\\\\\"ur das am Markt schon gut eingef\\\\\"uhrte Produkt abh\\\\\"angig?",seps="")
...
questions[1] <- paste("Wenn f\\\\\"ur ", product[3], if(paired) "mit Logo", " die durchschnittliche Bewertung durch die ", 
  ...
  if(paired) product[3], if(paired) "ohne Logo" else "das Parfum", " ungef\\\\\"ahr ", round(qu1,2), ".", seps="") 	
...
explanations[1] <- if(paired) paste("Die Differenz zwischen den durchschnittlichen Bewertungen mit und ohne 
  ...
  round(mean(dat[1:nobs,1]),2)-round(tt_paired$estimate,2),
  ".", seps="") else paste("Die Regressionsgerade geht immer durch den Mittelpunkt der Punktwolke. Daher kann man durch 
...
questions[3] <- paste("Die Daten k\\\\\"onnen geeignet visualisiert werden durch", vis_text[vis_index], seps="")
