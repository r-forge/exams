...
if(paired) {
  dat <- data.frame(Bewertung = score10(c(A, Bpaired)), Logo = factor(rep(1:2, c(nobs, nobs)), levels = 1:2, labels = c("alt", "neu")))
  # tt_paired <- t.test(Bewertung ~ Verpackung, data = dat, var.equal = TRUE, paired = TRUE)

  Verp.alt <- score10(Bcor) 
  dat1 <- list(Verp.alt=score10(A),Verp.fit=score10(Bpaired))
  tt_paired <- with(dat1,t.test(Verp.alt,Verp.fit,paired=TRUE))
  Verp.fit <- score10(A)
  fmod <- lm(Verp.alt~Verp.fit)
} else {  
  dat <- data.frame(Bewertung = score10(c(A, Bpaired)), Logo = factor(rep(1:2, c(nobs, nobs)), levels = 1:2, labels = c("alt", "neu")))
  # tt_paired <- t.test(Bewertung ~ Verpackung, data = dat, var.equal = TRUE, paired = TRUE)

  Joghurt <- score10(Bcor) 
  if(product==1)  
    {dat1 <- list(Joghurt=score10(A),Emmentaler=score10(Bpaired))
     tt_paired <- with(dat1,t.test(Joghurt,Emmentaler,paired=TRUE))
     Emmentaler <- score10(A)
     fmod <- lm(Joghurt~Emmentaler)} else 
  if(product==2)
    {dat1 <- list(Joghurt=score10(A),Gouda=score10(Bpaired))
     tt_paired <- with(dat1,t.test(Joghurt,Gouda,paired=TRUE))
     Gouda <- score10(A)
     fmod <- lm(Joghurt~Gouda)} else
  {dat1 <- list(Joghurt=score10(A),Butterkaese=score10(Bpaired))
     tt_paired <- with(dat1,t.test(Joghurt,Butterkaese,paired=TRUE)) 
     Butterkaese <- score10(A)
     fmod <- lm(Joghurt~Butterkaese)} 
}
...
product <- c("Emmentaler", "Gouda", "Butterk\\\\\"ase")[product]

study <- if(paired) paste("Ein K\\\\\"asehersteller verkauft seinen ", product, " momentan in einer 
  herk\\\\\"ommlichen Verpackung. Er vermutet, der K\\\\\"ase k\\\\\"onnte die Kunden besser anspechen, 
  wenn er ihn als Fitness-Produkt verpackt. Deshalb f\\\\\"uhrt er folgende Kundenbefragung durch: 
  Es werden", nobs, "Testpersonen ausgew\\\\\"ahlt. Zun\\\\\"achst l\\\\\"asst man sie den", product,
  "aus der alten herk\\\\\"ommlichen Verpackung kosten und fragt nach einer Bewertung auf einer Skala von 
  0 (schlecht) bis 10 (gut).", "Dann l\\\\\"asst man sie den K\\\\\"ase aus der neuen Fitness-Verpackung kosten und 
  fragt nach einer weiteren Bewertung. Der K\\\\\"asehersteller stellt nun folgende Frage: Unterscheidet sich 
  die mittlere Kundenbewertung f\\\\\"ur den ", product, "\\\\\"uberhaupt nach der Verpackung?") else
  paste("Ein K\\\\\"asehersteller  l\\\\\"asst seinen ", product, " von ", nobs, " seiner Kunden bewerten. 
  Er plant ein neues Produkt, n\\\\\"amlich einen Joghurtdrink, auf den Markt zu bringen und l\\\\\"asst 
  die ", nobs, " Kunden daher zus\\\\\"atzlich eine Bewertung f\\\\\"ur
  den Joghurtdrink abgeben. Beide Male steht eine Skala von 0 (schlecht) bis 10 (gut) zur Verf\\\\\"ugung. 
  Er interessiert sich daf\\\\\"ur, ob seine Kunden den neuen Joghurtdrink \\\\\"ahnlich bewerten wie den ", product,
  ". Der K\\\\\"asehersteller stellt nun folgende Frage: Ist die Bewertung f\\\\\"ur den neuen Joghurtdrink von der Bewertung
  f\\\\\"ur den ", product, " abh\\\\\"angig?",sep="")
...
prog <- rep(0,4)
prog[1] <- round(mean(dat[1:nobs,1]),2)-tt_paired$estimate
prog[2] <- predict(lm(Bprog~Aprog),newdata=data.frame(Aprog=round(mean(dat[1:nobs,1]),2)))
...
questions[1] <- paste("Wenn f\\\\\"ur den ", product, if(paired) " in der herk\\\\\"ommlichen Verpackung", 
  " die durchschnittliche Bewertung durch die ", 
  nobs, " Kunden gleich ", round(mean(dat[1:nobs,1]),2), " ist, dann betr\\\\\"agt die durchschnittliche Bewertung f\\\\\"ur den ",
  if(paired) product, if(paired) " in der Fitnessverpackung" else " Joghurtdrink", " ungef\\\\\"ahr ", round(qu1,2), ".", sep="") 	
solutions[1] <- if(paired) id == 1 else id == 2
explanations[1] <- if(paired) paste("Die Differenz zwischen den durchschnittlichen Bewertungen f\\\\\"ur die alte und die
  Fitnessverpackung betr\\\\\"agt ", round(tt_paired$estimate,4), ". Die durchschnittliche Bewertung f\\\\\"ur das Produkt 
  in Fitnessverpackung kann daher berechnet werden: (", 
  round(mean(dat[1:nobs,1]),2), ") - (", round(tt_paired$estimate,4), ") = ", 
  round(mean(dat[1:nobs,1]),2)-round(tt_paired$estimate,4),
  ...
  Wert f\\\\\"ur den Joghurtdrink ausrechnen: ", round(fmod$coef[1],4), " + (", round(fmod$coef[2],4), ") $\\\\cdot$ (", 
    round(mean(dat[1:nobs,1]),2), ") = ", round(prog[2],2), sep = "")  
   
				
questions[2] <- "Zur Beantwortung der Frage des K\\\\\"aseherstellers muss getestet werden, ob es einen Zusammenhang zwischen den beiden Bewertungen der Kunden gibt."
...
questions[3] <- paste("Die Daten k\\\\\"onnen durch ", vis_text[vis_index], "geeignet visualisiert werden.", sep="")
...
explanations[4] <- if(paired) paste("Die Frage ist, ob die mittlere Bewertung f\\\\\"ur den", product,
  "in der alten Verpackung dieselbe ist wie f\\\\\"ur den", product,
  "in der Fitnessverpackung.") else paste("Hier wird nach dem Zusammenhang und nicht nach 
...
<<echo=FALSE, results=verbatim>>=
print(fmod$call[[2]])
printCoefmat(tt_cor,signif.stars =FALSE)
@
