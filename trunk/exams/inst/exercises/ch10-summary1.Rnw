...
explanations[3] <- paste("Die Nullhypothese wird verworfen, wenn der zugeh\\\\\"orige $p$-Wert kleiner als $0.05$ ist. 
				Hier ist der $p$-Wert gleich ", format(round(summary(fm23)$coef[2,4],4),scientific=FALSE) ,".",sep="") 
    
## 4. testen der parameter gemeinsam
qu4 <- c("verworfen", "beibehalten")
questions[4] <- paste("Die Hypothese $H_0: \\\\beta_1 = \\\\beta_2 = 0$ wird verworfen 
...
explanations[5] <- paste("Die Nullhypothese wird verworfen, wenn der zugeh\\\\\"orige $p$-Wert kleiner als $0.05$ ist. 
				Hier ist der $p$-Wert gleich ", format(round(summary(fm23)$coef[3,4],4),scientific=FALSE) ,".",sep="") 
...
<<echo=FALSE>>=
printCoefmat(summary(fm23)$coefficients)
@
