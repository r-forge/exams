...
if (runif(1) > 0.5) {
  question0[[1]] <- "Je gr\\\\\"o{\\\\ss}er $\\\\alpha$, desto 
  besser die Gl\\\\\"attung."
  solutions[1]  <- FALSE
...
if (runif(1) > 0.5) {
  question0[[2]] <- "Eine Gl\\\\\"attung hat zum Ziel irregul\\\\\"are Effekte 
  ...
  question0[[2]] <- "Die exponentielle Gl\\\\\"attung liefert bei 
...
{\bf Erkl\"arungen:}\\
Die exponentiell gegl\"attete Reihe, $L_t$, ergibt sich 
als 
$$L_t = \alpha \cdot Y_t + (1-\alpha) \cdot (Y_{t-1} - L_{t-1})$$
Dabei gewichten $\alpha$ und $(1-\alpha)$ jeweils den aktuellen Wert bzw.\ 
den zuletzt gemachten Gl\"attungsfehler. Ist $\alpha$ gro{\ss}, so erh\"alt 
$Y_t$ ein gro{\ss}es Gewicht (aktuelle Werte sind wichtig)  und die weit 
zur\"uckliegenden --- sie sind in $L_{t-1}$ enthalten --- bekommen ein geringes 
