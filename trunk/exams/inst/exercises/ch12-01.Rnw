...
if (runif(1) > 0.5) {  
   questions[4]   <- "Damit eine Reihe als white noise identifiziert 
      werden kann , m\\\\\"ussen alle Autokorrelationskoeffizienten f\\\\\"ur $k > 0$ 
...
walk generiert. 
Zusammen mit den zugeh\"origen Korrelogrammen sind sie unten in 
zuf\"alliger Reihenfolge abgebildet. 
...
\begin{itemize}
 ...
 \item {\it Autogregressives Modell:} ZB ein AR(1) lautet 
    ...
    Werte sind iA (auto-)korreliert. Dadurch k\"onnen entweder l\"angere Zyklen, 
