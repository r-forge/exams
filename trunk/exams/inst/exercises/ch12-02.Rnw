...
sseries  <- vector("list",nsel)
kseries  <- vector("list",nsel)
...
sseries <- series[oy]
snames  <- names
snames[1] <- paste(names[1]," mit dem Korrelogramm ", sep="")
snames[2] <- paste(names[2],". Das zugehörige Korrelogramm ist ", sep="")
snames[3] <- paste(names[3],". Das zugehörige Korrelogramm ist ", sep="")
snames  <- snames[oy]
slabels <- labels[oy]
...
kseries <- series[ok]
klabels <- labels[ok]
knum <- match(oy,ok); knum <- paste("K",knum,sep=""); 

text <- " Das zugehörige Korrelogramm ist K"
ktext <- sample(1:3)
ktext1 <- paste("K",ktext,sep="")
ytext <- sample(1:3)
## QUESTION/ANSWER GENERATION
solutions <- c(F, F, F, F, F)

#questions[1]   <- paste("Y1 ",names[1],".",text,ktext[1],".",sep="")
questions[1] <- paste("Zur Reihe Y1 gehört das Korrelogramm ",
                ktext1[1],".",sep="")
if (slabels[1] == "WN" && klabels[ ktext[1] ] == "WN") {solutions[1] <- TRUE }  

#questions[2]   <- paste("Y2 ",names[2],"."text,ktext[2],".",sep="")
questions[2] <- paste("Zur Reihe Y2 gehört das Korrelogramm ",
                ktext1[2],".",sep="")
if (slabels[2] == "AR" && klabels[ ktext[2] ] == "AR") {solutions[2] <- TRUE }  

#questions[3]   <- paste("Y3 ",names[3],".",text,ktext[3],".",sep="")
questions[3] <- paste("Zur Reihe Y3 gehört das Korrelogramm ",
                ktext1[3],".",sep="")
if (slabels[3] == "RW" && klabels[ ktext[3] ] == "RW") {solutions[3] <- TRUE }  

questions[4]   <- paste("Reihe Y",ytext[1]," und Korrelogramm K",ytext[2]," 
   sind einander zuzuordnen.",sep="")
if (slabels[ ytext[1] ] == klabels[ ytext[2] ] ) {solutions[4] <- TRUE }  
                     
questions[5] <- "Keine der oben getroffenen Zuordnungen ist richtig."
...
\begin{question} 
  ...
  Es wurden je ein Pfad eines white noise, eines AR(1) und eines
  random walk generiert. Dazu wurden die Korrelogramme
  berechnet. Reihen und Korrelogramme sind unten - allerdings in einer
  {\it zuf\"alligen Anordnung} - graphisch dargestellt. Die Reihen,
  wie auch die Korrelogramme wurden zuf\"allig und unabh\"angig
  voneinander angeordnet.  Aufgabe ist es jeder Reihe das richtige
  Korrelogramm zuzuordnen.

\setkeys{Gin}{width=0.8\textwidth}
\begin{figure}[htb!]
\begin{center}
<<fig=TRUE, height = 6, width = 8, echo=FALSE, eps=FALSE, results=hide>>=
op <- par(mfrow = c(3, 2), mar = c(4.1, 4.1, 1.6, 1.6)) 
  ser <- sseries[[ 1 ]] 
   plot(ser,xlab="time",ylab="Y1") 
     if ( attr(ser,"tsp")[3] != 1 ) {  
     my.yax <- exams:::f_yax(ser)
     axis(1, at=my.yax, labels = FALSE, tick = TRUE, lwd = 1, tcl = -0.5) 
     my.sax <- exams:::f_sax(ser)
     axis(1, at=my.sax, labels = FALSE, tick = TRUE, lwd = 1, tcl = -0.25)
                                     } 

  acf(coredata(kseries[[1]]), plot = TRUE, ylab="ACF-K1")
   ...
   acf(coredata(kseries[[2]]), plot = TRUE, ylab="ACF-K2")
   ...
   acf(coredata(kseries[[3]]), plot = TRUE, ylab="ACF-K3")
...
\begin{itemize}
 ...
 \item {\it Autogregressives Modell:} ZB ein AR(1) lautet 
    ...
    $\epsilon_t$ WN.\\
    Autoregressive Modelle sind ebenfalls station\"ar. Aufeinanderfolgende 
    Werte sind iA (auto-)korreliert. Dadurch können entweder l\"angere 
    Zyklen, 
 ...
 \item {\it Random walk:} $y_t = c + y_{t-1} + \epsilon_t$, 
    ...
    eines gedachten, linearen Trends liegt. Diese sind wesentlich l\"anger 
    als in 
    einem AR(1)-Modell mit einem gro{\ss}en $\alpha$.\\
    Im Korrelogramm zeigen sich diese starken zeitlichen 
    Abh\"angigkeiten als ein langsamer, linearer Abfall.  
\end{itemize}

\vspace{0.3cm}

Die richtigen Zuordnungen der Reihen zu ihren Korrelogrammen sind:\\
Die Reihe Y1 \Sexpr{snames[1]} \Sexpr{knum[1]}.\\ 
Die Reihe Y2 \Sexpr{snames[2]} \Sexpr{knum[2]}.\\
Die Reihe Y3 \Sexpr{snames[3]} \Sexpr{knum[3]}.
