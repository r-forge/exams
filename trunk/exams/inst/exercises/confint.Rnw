...
\begin{question}
  The daily expenses of summer tourists in Vienna is analyzed. A
  survey with $\Sexpr{n}$ tourists is conducted. This gives that the
  tourists spend on average $\Sexpr{Mean}$ EUR. The data has a sample
  variance $s^2_{n-1}$ equal to $\Sexpr{Var}$.
