...
In the percentage table, the \Sexpr{rowsums[variant]} are about 100 (except for
possible rounding errors). Hence, the table provides
\Sexpr{percentage[variant]} percentages, i.e., conditional proportions
for FIXME given FIXME.
This signifies that \Sexpr{interpretation[variant]}
